export default {
  common: {
    welcome: "Chào mừng",
    login: "Đăng nhập",
    logout: "Đăng xuất",
    register: "Đăng ký",
    signIn: "Đăng nhập",
    or: "Hoặc",
    and: "và",
    email: "Email",
    emailAddress: "Địa chỉ Email",
    password: "Mật khẩu",
    confirmPassword: "Xác nhận mật khẩu",
    createAccount: "Tạo tài khoản",
    alreadyHaveAccount: "Đã có tài khoản?",
    phoneNumber: "Số điện thoại",
    firstName: "Tên",
    lastName: "Họ",
    support: "Hỗ trợ",
    unknown: "Không xác định",
    back: "Quay lại",
    notification: "Thông báo",
    language: "Ngôn ngữ",
    welcomeTo: "Chào mừng đến",
    loading: "Đang tải...",
    seeAll: "Xem tất cả ",
    seeYourOffers: "Xem ưu đãi của bạn",
    discount: "Giảm",
    installment: "Trả góp",
    productFilter: "Bộ lọc",
    sortBy: "Sắp xếp",
    qa: "Hỏi đáp",
    productCommitment: "Cam kết sản phẩm",
    authenticApple: "Chính hãng Apple",
    exchange30days: "Đổi trả 30 ngày",
    activateWarranty: "Kích hoạt bảo hành",
    boxIncludes: "Hộp bao gồm",
    vatIncluded: "Đã bao gồm VAT",
    showAllSpecs: "Xem tất cả thông số",
    installment0: "Trả góp 0%",
    buyNowAction: "Mua ngay",
    viewProductInfo: "Xem thông tin sản phẩm",
    seeMore: "Xem thêm",
    viewMoreProducts: "Xem thêm {count} sản phẩm",
    comboSelect: "Chọn combo",
    profile: "Tài khoản",
    orders: "Đơn hàng",
    wishlist: "Yêu thích",
    save: "Lưu",
    cancel: "Huỷ",
    close: "Đóng",
    remove: "Xoá",
    confirmNo: "Không",
    confirmYes: "Xoá",
  },
  orders: {
    title: "Quản lý đơn hàng",
    subtitle: "Theo dõi và cập nhật trạng thái đơn hàng",
    filters: {
      search: "Tìm kiếm",
      searchPlaceholder: "Tìm theo mã đơn, tên khách hàng hoặc email...",
      status: "Trạng thái",
      allStatus: "Tất cả trạng thái",
    },
    table: {
      orderId: "Mã đơn",
      customer: "Khách hàng",
      items: "Sản phẩm",
      total: "Tổng tiền",
      status: "Trạng thái",
      date: "Ngày đặt",
      actions: "Thao tác",
    },
    noOrders: "Chưa có đơn hàng nào",
  },
  search: {
    confirmClearHistory: "Xoá toàn bộ lịch sử tìm kiếm?",
    suggestionsTitle: "Có phải bạn muốn tìm",
    suggestedProducts: "Sản phẩm gợi ý",
    noHistory: "Bạn chưa có lượt tìm kiếm gần đây.",
    searchHistory: "Lịch sử tìm kiếm",
    trendingSearch: "Xu hướng tìm kiếm",
  },
  wishlist: {
    title: "Sản phẩm yêu thích",
    emptyTitle: "Chưa có sản phẩm yêu thích",
    emptyDesc: "Hãy thêm sản phẩm bạn thích vào đây nhé!",
    shopNow: "Mua sắm ngay",
    addToCart: "Thêm vào giỏ",
    remove: "Xóa",
    confirmRemove: 'Xoá "{name}" khỏi yêu thích?',
    removed: "Đã xóa khỏi yêu thích",
    removeError: "Không thể xóa sản phẩm",
    addedToCart: "Đã thêm vào giỏ hàng",
    addToCartError: "Không thể thêm vào giỏ hàng",
    loginRequired: "Vui lòng đăng nhập để xem wishlist",
  },
  notification: {
    title: "Thông báo",
    markAllRead: "Đánh dấu đã đọc",
    loading: "Đang tải...",
    loadError: "Không thể tải thông báo",
    empty: "Không có thông báo",
    removeAria: "Xoá thông báo",
  },
  role: {
    default: "Mặc định",
    admin: "Quản trị viên",
    manager: "Quản lý",
    customer: "Khách hàng",
    new_customer: "Khách hàng mới",
    accountant: "Kế toán",
    courier: "Người giao hàng",
  },
  sidebar: {
    brandName: "IrusGear",
    search: "Tìm kiếm",
    searchPlaceholder: "Tìm kiếm...",
    mobileCategory: {
      open: "Mở danh mục",
      close: "Đóng danh mục",
      title: "Danh mục",
      ariaLabel: "Danh mục sản phẩm",
      rootAriaLabel: "Danh mục chính",
      loading: "Đang tải danh mục...",
      loadError: "Không thể tải danh mục",
      viewAll: "Xem tất cả {title}",
      fallbackTitle: "Danh mục",
    },
    sections: {
      main: "Chính",
      communication: "Liên lạc",
    },
    menu: {
      home: "Trang chủ",
      orders: "Đơn hàng",
      documentation: "Tài liệu",
      mapOverview: "Bản đồ",
      statistics: "Thống kê",
      inbox: "Hộp thư",
      couriers: "Người giao hàng",
      settings: "Cài đặt",
      help: "Trợ giúp",
      products: "Sản phẩm",
    },
    capacity: {
      title: "Dung lượng sử dụng",
      description: "Bạn đã sử dụng {percent}% dung lượng.",
      upgradeBtn: "Nâng cấp",
    },
  },
  page_titles: {
    home: "Trang chủ",
    login: "Đăng nhập",
    forgotPassword: "Quên mật khẩu",
    register: "Đăng ký",
    dashboard: "Bảng điều khiển",
    verify: "Xác minh tài khoản",
  },
  dashboard: {
    welcome: "Chào mừng trở lại",
    overview: "Tổng quan",
    stats: {
      totalRevenue: "Tổng doanh thu",
      totalOrders: "Tổng đơn hàng",
      totalCustomers: "Tổng khách hàng",
      totalProducts: "Tổng sản phẩm",
      vsLastMonth: "so với tháng trước",
    },
    recentOrders: {
      title: "Đơn hàng gần đây",
      viewAll: "Xem tất cả",
      orderId: "Mã đơn",
      customer: "Khách hàng",
      date: "Ngày",
      amount: "Số tiền",
      status: "Trạng thái",
      noOrders: "Chưa có đơn hàng nào",
    },
    orderStatus: {
      pending: "Chờ xử lý",
      processing: "Đang xử lý",
      shipped: "Đã giao",
      delivered: "Hoàn thành",
      cancelled: "Đã hủy",
    },
    salesChart: {
      title: "Doanh thu theo tháng",
      revenue: "Doanh thu",
    },
    topProducts: {
      title: "Sản phẩm bán chạy",
      product: "Sản phẩm",
      sold: "Đã bán",
      revenue: "Doanh thu",
    },
  },
  login: {
    newUser: "Người dùng mới?",
    createAccount: "Tạo tài khoản",
    forgotPassword: "Quên mật khẩu của bạn?",
    joinSocial: "Tham gia với tài khoản mạng xã hội yêu thích của bạn",
    terms: "Bằng cách đăng nhập bằng tài khoản, bạn đồng ý với các",
    termsLink: "Điều khoản dịch vụ",
    privacyLink: "Chính sách bảo mật",
    rememberMe: "Ghi nhớ đăng nhập",
    memberTitle: "Trở thành thành viên",
    memberSubtitle: "Để không bỏ lỡ các ưu đãi hấp dẫn",
    benefitDiscount: "Chiết khấu đến 5%",
    benefitDiscountDesc: "khi mua các sản phẩm tại IRUS Gear",
    benefitShipping: "Miễn phí giao hàng",
    benefitShippingDesc: "cho thành viên VIP và đơn hàng từ 500.000đ",
    benefitBirthday: "Tặng voucher sinh nhật",
    benefitBirthdayDesc: "đến 300.000đ cho khách hàng thành viên",
    benefitTradeIn: "Trợ giá thu cũ lên đời",
    benefitTradeInDesc: "đến 500.000đ",
    benefitRankUp: "Thăng hạng nhận voucher",
    benefitRankUpDesc: "đến 200.000đ",
    benefitBusiness: "Đặc quyền doanh nghiệp:",
    benefitBusinessDesc: "Chiết khấu đến 10% dành riêng cho khách hàng B2B",
    viewMemberPolicy: "Xem chi tiết chính sách ưu đãi thành viên",
    loginSuccess: "Đăng nhập thành công!",
  },
  register: {
    title: "Đăng ký",
    subtitle: "Trở thành thành viên ngay hôm nay",
    quickRegister: "Đăng ký nhanh bằng",
    orInfo: "Hoặc điền thông tin",
    personalInfo: "Thông tin cá nhân",
    fullName: "Họ và tên",
    enterFullName: "Nhập họ và tên",
    dob: "Ngày sinh",
    phone: "Số điện thoại",
    enterPhone: "Nhập số điện thoại",
    email: "Email",
    enterEmail: "Nhập email",
    security: "Bảo mật",
    password: "Mật khẩu",
    confirmPassword: "Xác nhận mật khẩu",
    enterPassword: "Mật khẩu",
    reEnterPassword: "Nhập lại mật khẩu",
    promo: "Nhận tin khuyến mãi từ IrusGear",

    roleSwitch: "Học sinh - Sinh viên / Giáo viên",
    youAre: "Bạn là:",
    student: "Học sinh - SV",
    teacher: "Giáo viên",
    schoolLevel: "Cấp học",
    chooseLevel: "Chọn cấp học",
    university: "Đại học / Cao đẳng",
    highschool: "Trung học phổ thông",
    secondary: "Trung học cơ sở",
    schoolName: "Tên trường",
    enterSchoolName: "Nhập tên trường",
    uploadID: "Tải ảnh thẻ (Mặt trước & sau)",
    frontSide: "Mặt trước",
    backSide: "Mặt sau",
    uploadDragDrop: "Kéo thả hoặc",
    uploadSelect: "Chọn file",
    uploadFormat: "Định dạng: .JPG, .PNG, .PDF - Tối đa 5 MB",

    businessSwitch: "Tôi là Khách hàng Doanh nghiệp",
    taxCode: "Mã số thuế",
    enterTaxCode: "Nhập mã số thuế",
    companyName: "Tên công ty",
    enterCompanyName: "Nhập tên công ty",
    companyAddress: "Địa chỉ công ty",
    enterCompanyAddress: "Nhập địa chỉ công ty",
    companyEmail: "Địa chỉ email công ty",
    enterCompanyEmail: "Nhập địa chỉ email công ty",
    authorizedCCCD: "CCCD người được uỷ quyền",
    enterAuthorizedCCCD: "Nhập CCCD người được uỷ quyền",
    note: "Ghi chú",
    optional: "(Không bắt buộc)",
    enterNote: "Nhập ghi chú",
    businessLicense: "Giấy phép kinh doanh",
    authorizationLetter: "Giấy uỷ quyền",
    viewSample: "Xem file mẫu",

    backBtn: "Quay lại",
    registerBtn: "Đăng ký",
    agreePolicy: "Bằng việc đăng ký, bạn đồng ý với",
    terms: "Điều khoản",
    and: "&",
    policy: "Chính sách",
    backToLogin: "Quay lại đăng nhập",
    goBack: "Quay lại",
    rememberPassword: "Bạn nhớ mật khẩu của mình?",
    businessCustomer: "Khách hàng doanh nghiệp",
    register: "Đăng ký",
    agreeWith: "Tôi đồng ý với",
    uploadAction: "Tải file lên",
  },
  footer: {
    slogan: {
      line1: "Gear tốt nhất",
      line2: "cho phong cách",
      line3: "của bạn",
    },
    locations: {
      hcm: {
        name: "Hồ Chí Minh",
        address1: "123 Nguyễn Huệ,",
        address2: "Quận 1, TP.HCM",
      },
      hanoi: {
        name: "Hà Nội",
        address1: "456 Phố Huế,",
        address2: "Hai Bà Trưng, Hà Nội",
      },
    },
    seeOnMap: "XEM BẢN ĐỒ",
    newsletter: {
      question: "Muốn nhận tin mới nhất?",
      signup: "ĐĂNG KÝ NHẬN TIN",
    },
    followUs: "Theo dõi chúng tôi",
    copyright: "© 2026 IrusGear. Bảo lưu mọi quyền.",
    description:
      "Nền tảng mua sắm công nghệ cao cấp. Sản phẩm chính hãng, bảo hành minh bạch, trải nghiệm mượt mà từ UI đến dịch vụ.",
  },
  validate: {
    passwordsNotMatch: "Mật khẩu không khớp",
    required: "Vui lòng nhập thông tin này",
    invalid_date: "Ngày không hợp lệ",
    numeric_only: "Vui lòng chỉ nhập số",
    phone_min: "Số điện thoại phải có ít nhất 10 số",
    phone_max: "Số điện thoại không được quá 12 số",
    email_invalid: "Email không đúng định dạng",
    password_min: "Mật khẩu phải có ít nhất 6 ký tự",
    password_not_match: "Mật khẩu xác nhận không khớp",
    max_length: "Nội dung không được vượt quá {max} ký tự",
    xss_invalid:
      "Vui lòng không nhập các ký tự đặc biệt (&lt;, &gt;, &#123;, &#125;, ;)",
    password_confirm_required: "Mật khẩu xác nhận không khớp",
    invalid_email: "Email không hợp lệ",
    otp_required: "Vui lòng nhập đủ 6 số.",
  },
  error: {
    oops: "Oops",
    pageNotFound: "Trang bạn tìm kiếm không tồn tại",
    cantFind: "chúng tôi không tìm thấy trang đó...",
    dragMe: "kéo tôi",
    backHome: "Về trang chủ",
  },
  verify: {
    title: "Xác minh tài khoản",
    subtitle: "Vui lòng nhập mã xác minh đã được gửi đến email của bạn",
    description: "Mã xác thực đã được gửi đến email",
    otpSubtitle: "Nhập mã xác thực",
    verifyBtn: "Xác minh",
    resendCode: "Bạn chưa nhận được mã? ",
    resendBtn: "Gửi lại",
  },
  restorePassword: {
    recoverTitle: "Khôi phục mật khẩu",
    recoverSubtitle: "Nhập email đã đăng ký để nhận mã xác minh",
    checkSpam: "Kiểm tra hộp thư Spam",
    checkSpamDesc:
      "Nếu không thấy mã trong Hộp thư đến, hãy kiểm tra mục Spam hoặc Quảng cáo.",
    secureTip: "Bảo mật thông tin",
    secureTipDesc:
      "Tuyệt đối không chia sẻ mã xác thực (OTP) cho bất kỳ ai, kể cả nhân viên hỗ trợ.",
    needHelp: "Cần hỗ trợ thêm?",
    needHelpDesc: "Liên hệ cho chúng tôi nếu bạn không thể truy cập email này.",
    recover: "Khôi phục",
    forgotPassword: "@:page_titles.forgotPassword",
    verification: "Xác minh",
    resetPassword: "Đặt lại mật khẩu",
    forgotPasswordSubtitle: "Đừng lo lắng, chúng tôi ở đây để hỗ trợ bạn.",
    otpSubtitle: "Vui lòng nhập mã xác minh đã được gửi đến email của bạn",
    newPasswordSubtitle: "Vui lòng nhập mật khẩu mới cho tài khoản của bạn",
    forgotPasswordNote: "Chúng tôi sẽ gửi mã xác nhận đến email của bạn.",
    sendCode: "Gửi mã xác nhận",
    newPassword: "Mật khẩu mới",
    confirmNewPassword: "Xác nhận mật khẩu mới",
    resetPasswordBtn: "Đổi mật khẩu",
  },
  cart: {
    cart: "Giỏ hàng",
    cartOf: "Giỏ hàng của bạn",
    addedToCart: "Đã thêm sản phẩm vào giỏ",
    quantity: "Số lượng",
    price: "Giá",
    moreItems: "sản phẩm khác",
    productsInCart: "{count} sản phẩm trong giỏ",
    total: "Thành tiền",
    subtotal: "Tạm tính",
    saving: "Bạn đang tiết kiệm {amount} so với mức giá đã lưu trước đó.",
    cartRetainNote:
      "Giỏ của bạn sẽ được giữ lại khi đăng nhập hoặc quay lại trên cùng trình duyệt.",
    continueShopping: "Mua tiếp",
    viewCart: "Xem giỏ hàng",
    emptyTitle: "Giỏ hàng của bạn đang trống",
    emptyDescription:
      "Hãy chọn thêm sản phẩm để bắt đầu mua sắm nhé. Mình gợi ý vài lối đi nhanh để bạn quay lại đúng khu vực đang quan tâm.",
    exploreProducts: "Khám phá sản phẩm",
    laptop: "Laptop",
    audio: "Âm thanh",
    watch: "Đồng hồ",
    accessory: "Phụ kiện",
    removeFromCart: "Xóa sản phẩm khỏi giỏ",
    decreaseQuantity: "Giảm số lượng sản phẩm",
    increaseQuantity: "Tăng số lượng sản phẩm",
    stoppedSelling: "Ngừng bán",
    outOfStock: "Hết hàng",
    newPrice: "Giá mới",
    checkCart: "Cần kiểm tra lại giỏ hàng",
    checkNow: "Kiểm tra ngay",
    productRemoved:
      "Sản phẩm đã được lấy khỏi giỏ. Bạn có thể hoàn tác ngay nếu vừa thao tác nhầm.",
    undo: "Hoàn tác",
    productList: "Danh sách sản phẩm",
    updateQuantityNote:
      "Bạn có thể cập nhật số lượng trực tiếp tại đây. Hệ thống sẽ làm mới giá và thành tiền ngay sau mỗi thao tác.",
    selectItem: "Chọn sản phẩm này",
    selectAll: "Chọn tất cả",
    selectedProducts: "Đã chọn",
    removeSelected: "Xoá đã chọn",
    confirmRemoveSelected: "Xoá {count} sản phẩm đã chọn khỏi giỏ?",
    removeSelectedSuccess: "Đã xoá các sản phẩm đã chọn.",
    noItemSelected: "Vui lòng chọn ít nhất 1 sản phẩm để thanh toán.",
    noItemSelectedHint: "Tick chọn sản phẩm bạn muốn thanh toán ngay bây giờ.",
    selectItemsToCheckout: "Chọn sản phẩm để thanh toán",
    loadingCart: "Đang tải giỏ hàng...",
    cartProgress: "Tiến trình giỏ hàng",
    orderInfo: "Thông tin đặt hàng",
    checkout: "Thanh toán",
    complete: "Hoàn tất",
    cartSummary: "Thông tin giỏ hàng",
    products: "Sản phẩm",
    delivery: "Giao hàng",
    deliveryNote: "Tính ở bước tiếp theo",
    savingAmount: "Tiết kiệm",
    emptyCart: "Giỏ đang trống",
    proceedOrder: "Tiến hành đặt hàng",
    checkCartItems: "Kiểm tra giỏ hàng",
    addProductNextStep: "Thêm sản phẩm để bắt đầu bước tiếp theo.",
    someItemsNotReady:
      "Một số sản phẩm chưa sẵn sàng. Mình sẽ đưa bạn tới đúng vị trí cần kiểm tra trong giỏ.",
    nextStepNote:
      "Luồng thông tin nhận hàng và thanh toán sẽ được nối sang bước tiếp theo của dự án.",
    updateQuantityError: "Không thể cập nhật số lượng sản phẩm.",
    confirmRemoveItem: 'Xoá "{name}" khỏi giỏ hàng?',
    confirmClearCart: "Xoá tất cả sản phẩm khỏi giỏ hàng?",
    removeFromCartError: "Không thể xóa sản phẩm khỏi giỏ hàng.",
    restoreToCart: "Đã khôi phục sản phẩm vào giỏ hàng.",
    restoreError: "Không thể khôi phục sản phẩm vừa xóa.",
    checkoutNote:
      "Luồng checkout đang được hoàn thiện. Cart đã sẵn sàng để nối sang bước thông tin và thanh toán ở phase tiếp theo.",
    clearCart: "Giỏ hàng đã được làm trống.",
    clearCartError: "Không thể làm trống giỏ hàng.",
    clearAll: "Xóa tất cả",
    continueBuy: "Tiếp tục mua",
    itemCountWarning:
      "{count} sản phẩm hiện đang hết hàng hoặc không còn sẵn sàng để bán. Vui lòng rà soát lại trước khi tiếp tục.",
    priceStockChange:
      "{count} sản phẩm đang có thay đổi về giá hoặc trạng thái tồn kho.",
    productRemovedTitle: "Đã xóa sản phẩm khỏi giỏ",
    productRemovedTitleWithName: "Đã xóa {name} khỏi giỏ",
    someItemsWarning:
      "Một vài sản phẩm đang thay đổi về giá hoặc tồn kho. Bạn nên kiểm tra lại trước khi tiếp tục.",
    trustNote1: "Giữ nguyên giỏ khi đăng nhập và chuyển thiết bị.",
    trustNote2: "Giá và tồn kho sẽ được đồng bộ ngay trên giỏ.",
    trustNote3:
      "Phù hợp với phong cách mua sắm retail điện tử hiện tại của site.",
    suggestionForYou: "Gợi ý cho bạn",
    buyMoreCompleteSet: "Mua thêm cho trọn bộ",
    suggestionDesc:
      "Mình đang ưu tiên các combo và phụ kiện phù hợp với sản phẩm đang có trong giỏ.",
    loadingSuggestions: "Đang tải gợi ý phù hợp cho giỏ hàng...",
    contact: "Liên hệ",
    comboPrice: "Mua kèm giá tốt",
    comboSuggestion: "{count} gợi ý",
    comboAccessory: "Phụ kiện phù hợp",
    addNow: "Thêm ngay",
    addToCart: "Thêm vào giỏ",
    bundleDeal: "Ưu đãi mua kèm",
    suggestedAccessory: "Phụ kiện gợi ý",
    memberDiscount: "Thành viên giảm thêm đến {amount}",
    suggestionAdded: "Đã thêm gợi ý vào giỏ hàng.",
    productLines: "Dòng hàng",
    hintSave: "Tiết kiệm {amount}",
    productNotFound: "Không tìm thấy sản phẩm để thêm vào giỏ.",
    outOfStockForCart: "Đã hết hàng cho sản phẩm này.",
    addToCartDisabled: "Sản phẩm hiện chưa thể thêm vào giỏ hàng.",
    variantNotSelected: "Không thể xác định phiên bản sản phẩm đang chọn.",
    addToCartSuccess: "Đã thêm vào giỏ hàng.",
    productNotReady: "Sản phẩm chưa sẵn sàng để thêm vào giỏ.",
  },
  product: {
    buyNow: "Mua ngay",
    addToCart: "Thêm vào giỏ",
    favorite: "Yêu thích",
    addedToWishlist: "Đã thêm vào yêu thích",
    removedFromWishlist: "Đã xóa khỏi yêu thích",
    wishlistError: "Không thể cập nhật yêu thích",
    confirmRemoveFromWishlist: 'Xoá "{name}" khỏi yêu thích?',
    compare: "So sánh",
    specs: "Thông số",
    color: "Màu sắc",
    version: "Phiên bản",
    details: "Chi tiết",
    warranty: "Bảo hành",
    tradeInFrom: "Thu cũ lên đời chỉ từ",
    tradeInSupport: "Trợ giá đến",
    rateNow: "Định giá ngay",
    memberSaving: "Tiết kiệm lên đến",
    forSmember: "cho Thành viên",
    checkNow: "Kiểm tra ngay",
    or: "Hoặc",
    promotionIncluded: "Khuyến mãi đi kèm",
    close: "Đóng",
    creditCardRefund: "Hoàn tiền đến 2 triệu khi mở thẻ tín dụng HSBC",
    warranty12Months:
      "Bảo hành 12 tháng tại trung tâm bảo hành chính hãng Apple:",
    warrantyDetailAt: "Chi tiết tại",
    vatRefund: "hoàn thuế VAT - Tax Refund",
    specsTitle: "Thông số kỹ thuật",
    showMoreSpecs: "Xem thêm {count} thông số",
    specsUpdating: "Thông số kỹ thuật đang được cập nhật.",
    warranty1to1:
      "Bảo hành 1 đổi 1 trong 12 tháng nếu sản phẩm gặp lỗi từ nhà sản xuất.",
    appleCareWarranty: "Dịch vụ bảo hành Apple Care+ cho iPhone",
    extendedWarranty18:
      "Bảo hành mở rộng 18 tháng: Miễn phí sửa chữa các lỗi phần cứng từ NSX",
    reviewTitle: "Đánh giá {name}",
    reviewByExperience: "Đánh giá theo trải nghiệm",
    writeReview: "Viết đánh giá",
    reviewCount: "lượt đánh giá",
    reviews: "đánh giá",
    highReviews: "Đánh giá cao",
    lowReviews: "Đánh giá thấp",
    seeMoreReviews: "Xem thêm đánh giá",
    noReviewsTitle: "Chưa có đánh giá nào",
    noReviewsText: "Hãy là người đầu tiên chia sẻ cảm nhận thật về sản phẩm này.",
    noMatchingReviewsTitle: "Không có đánh giá phù hợp",
    noMatchingReviewsText: "Hãy thử chọn bộ lọc khác để xem thêm đánh giá.",
    qaTitle: "Hỏi và đáp",
    askQuestion: "Hãy đặt câu hỏi cho chúng tôi",
    writeQuestionPlaceholder: "Viết câu hỏi của bạn tại đây",
    writeReplyPlaceholder: "Viết phản hồi của bạn",
    fullNamePlaceholder: "Họ tên",
    sendQuestion: "Gửi câu hỏi",
    sending: "Đang gửi",
    seeMoreQuestions: "Xem thêm câu hỏi",
    similarProducts: "Sản phẩm tương tự",
    usedRef: "Hàng secondhand",
    mayAlsoLike: "Có thể bạn cũng thích",
    seeAll: "Xem tất cả",
    newsProduct: "Tin tức sản phẩm",
    comboPriceLabel: "Mua kèm giá sốc",
    rating: "đánh giá",
    joinProgram: "Tham gia chương trình",
    enterPhoneToJoin:
      "Vui lòng nhập số điện thoại để tham gia chương trình ưu đãi cho khách hàng mới.",
    continue: "Tiếp tục",
    filterReviews: "Lọc đánh giá",
    all: "Tất cả",
    star: "sao",
    sortLatest: "Mới nhất",
    helpful: "Hữu ích",
    reportReview: "Báo cáo",
    reportContentPlaceholder: "Mô tả thêm nếu cần",
    submitReport: "Gửi báo cáo",
    cancelReport: "Hủy",
    reportSpam: "Spam hoặc quảng cáo",
    reportIrrelevant: "Không liên quan sản phẩm",
    reportOffensive: "Ngôn từ không phù hợp",
    reportPersonalInfo: "Lộ thông tin cá nhân",
    reportOther: "Lý do khác",
    reviewHelpfulSuccess: "Cảm ơn bạn đã đánh dấu hữu ích.",
    reviewReportSuccess: "Đã ghi nhận báo cáo của bạn.",
    reviewActionFailed: "Không thể xử lý thao tác đánh giá.",
    verifiedPurchaseBadge: "Đã mua hàng",
    expPerformance: "Hiệu năng",
    expCamera: "Chất lượng camera",
    expBattery: "Thời lượng pin",
    expDesign: "Thiết kế",
    expScreen: "Màn hình",
    ratingExcellent: "Tuyệt vời",
    ratingVeryGood: "Rất tốt",
    ratingGood: "Tốt",
    ratingOkay: "Tạm được",
    ratingBad: "Chưa tốt",
    sendReply: "Gửi phản hồi",
    questionSuccess: "Gửi câu hỏi thành công.",
    descriptionUpdating: "Đang cập nhật mô tả...",
    discount: "Giảm",
    viewOnMap: "Bản đồ",
    shippingInfo: "Thông tin vận chuyển",
    selectAddressForOffer: "Chọn địa chỉ giao hàng để nhận ưu đãi",
    newLabel: "Mới",
    fastDeliveryNote: "Giao nhanh từ 2 giờ hoặc nhận tại cửa hàng",
    tradeInIncluded: "(Bao gồm Thành viên)",
    tradeInPrivilege:
      "Đặc quyền trợ giá lên đến 3 triệu khi thu cũ lên đời iPhone",
    simOffer: "Tặng Sim/Esim 5G VNSKY, có ngay 3GB data/ngày+500 phút gọi",
    simOfferDetail:
      "Mobifone & VNSKY, miễn phí 30 ngày đầu - chỉ áp dụng tại cửa hàng",
    accessoryDiscount:
      "Giảm thêm 10% cho Pin dự phòng - Camera - Gia dụng - Sức khỏe Làm đẹp - Đồng hồ trẻ em khi mua Điện thoại/Laptop",
    otpPrompt: "Vui lòng nhập OTP vừa được gửi đến số điện thoại",
    paymentOffers: "Ưu đãi thanh toán",
    paymentOffer1: "Giảm ngay 700K khi trả góp qua thẻ TECHCOMBANK",
    paymentOffer2: "Giảm 500K khi thanh toán qua thẻ tín dụng OCB",
    paymentOffer3: "Giảm đến 5.000.000đ khi thanh toán qua Kredivo",
    paymentOffer4: "Mở thẻ VIB nhận E-Voucher đến 600K",
    paymentOffer5: "Giảm đến 500K khi mở thẻ TPBANK EVO",
    ongoing: "Đang diễn ra",
    viewBranches: "Xem chi nhánh có hàng",
    inStock: "Có",
    storesWithProduct: "cửa hàng có sản phẩm",
    selectDistrict: "Quận/Huyện",
    hoChiMinh: "Hồ Chí Minh",
    district1: "Quận 1",
    district2: "Quận 2",
    district3: "Quận 3",
    district4: "Quận 4",
    district5: "Quận 5",
    district6: "Quận 6",
    district7: "Quận 7",
    district8: "Quận 8",
    district9: "Quận 9",
    district10: "Quận 10",
    district11: "Quận 11",
    district12: "Quận 12",
    binhThanh: "Quận Bình Thạnh",
    goVap: "Quận Gò Vấp",
    phuNhuan: "Quận Phú Nhuận",
    tanBinh: "Quận Tân Bình",
    tanPhu: "Quận Tân Phú",
    binhTan: "Quận Bình Tân",
    thuDuc: "Quận Thủ Đức",
    warrantyTab: "Bảo hành",
    warrantyVip6m: "1 đổi 1 VIP 6 tháng",
    warrantyVip6mDesc:
      "Đổi máy mới tương đương khi có lỗi từ NSX trong 6 tháng",
    warrantyS24p12m: "S24+ 12 tháng",
    warrantyS24p12mDesc:
      "Đổi sản phẩm tương đương hoặc miễn phí chi phí sửa chữa nếu có lỗi của NSX khi hết hạn bảo hành",
    warrantyVip12m: "1 đổi 1 VIP 12 tháng",
    warrantyVip12mDesc:
      "Bảo hành 1 đổi 1 trong 12 tháng nếu sản phẩm gặp lỗi từ nhà sản xuất.",
    warrantyBreakage12m: "Rơi vỡ rớt nước 12 tháng",
    warrantyBreakage12mDesc:
      "Hỗ trợ thêm cho các sự cố rơi vỡ, vào nước trong 12 tháng đầu sử dụng.",
    warrantyVip24m: "1 đổi 1 VIP 24 tháng",
    warrantyVip24mDesc:
      "Nâng thời gian bảo vệ lên 24 tháng với quyền đổi máy tương đương khi có lỗi từ NSX.",
    warrantyBreakage24m: "Rơi vỡ rớt nước 24 tháng",
    warrantyBreakage24mDesc:
      "Bảo vệ dài hạn cho các rủi ro rơi vỡ, vào nước trong 24 tháng.",
    warrantyAppleCare: "Dịch vụ bảo hành Apple Care+ cho iPhone",
    warrantyAppleCareDesc:
      "Gói dịch vụ Apple Care+ dành cho iPhone với quyền lợi bảo hành và hỗ trợ chính hãng.",
    warrantyExtend18m: "Bảo hành mở rộng 18 tháng",
    warrantyExtend18mDesc:
      "Mở rộng thời gian bảo hành thêm 18 tháng cho các lỗi phần cứng đủ điều kiện.",
    warrantyFull12m: "Gói toàn diện 12 tháng",
    warrantyFull12mDesc:
      "Kết hợp bảo hành lỗi từ nhà sản xuất và hỗ trợ một phần chi phí khi có sự cố ngoài ý muốn.",
    selectWarrantyPlan: "Chọn gói dịch vụ bảo hành",
    warrantyInfo: "Thông tin chi tiết về các gói bảo hành mở rộng.",
    questionReplyInfo:
      "IrusGear sẽ phản hồi trong vòng 1 giờ. Nếu Quý khách gửi câu hỏi sau 22h, chúng tôi sẽ trả lời vào sáng hôm sau.",
    questionNote:
      "Thông tin có thể thay đổi theo thời gian, vui lòng đặt câu hỏi để nhận được cập nhật mới nhất!",
    reply: "Phản hồi",
    collapseReplies: "Thu gọn phản hồi",
    viewReplies: "Xem {count} phản hồi",
    errMinQuestionChars: "Vui lòng nhập ít nhất 10 ký tự cho câu hỏi.",
    errSubmitQuestionFailed: "Không thể gửi câu hỏi.",
    errNoReplyContent: "Vui lòng nhập nội dung phản hồi.",
    errSubmitReplyFailed: "Không thể gửi phản hồi.",
    cdtInfo: "Thông tin CDTs",
    cdtAddress: "Địa chỉ cửa hàng",
    cdtPhone: "Hotline",
    cdtWorkingHours: "Giờ làm việc",
    reviewCountLabel: "{count} đánh giá",
    tradeInTitle: "Thu cũ lên đời",
  },
  home: {
    searchProducts: "Bạn muốn mua gì hôm nay?",
    searchShortcut: "Ctrl K",
    dealHot: "Deal Hot",
    home: "Trang chủ",
    news: "Tin tức",
    promotion: "Khuyến mãi",
    contact: "Liên hệ",
    category: "Danh mục",
    hotDeal: "Khuyến mãi",
    newsTitle: "TIN TỨC",
    or: "hoặc",
    featuredPromotions: "Ưu đãi nổi bật",
    educationPromo: "Ưu đãi cho giáo dục",
    registerPromo: "Đăng ký nhận ưu đãi",
    hotDealStudent: "Deal hot học sinh sinh viên",
    productNews: "Tin tức sản phẩm",
    loading: "Đang tải...",
    memberPromoNote: "Nhập hội thành viên để không bỏ lỡ các ưu đãi hấp dẫn.",
    viewSmemberOffers: "Xem ưu đãi Thành viên",
    catPhoneTablet: "Điện thoại, Tablet",
    catAudioMic: "Âm thanh, Mic thu âm",
    catWatchCamera: "Đồng hồ, Camera",
    catWatch: "Đồng hồ",
    catHomeAppliances: "Đồ gia dụng, Làm đẹp",
    catPcMonitorPrinter: "PC, Màn hình, Máy in",
    catTvAppliances: "Tivi, Điện máy",
    catTradeIn: "Thu cũ đổi mới",
    catUsedGoods: "Hàng cũ",
    catTechNews: "Tin công nghệ",
    registerPromoLabel: "Đăng ký nhận ưu đãi",
    emailPlaceholder: "Email của bạn",
    firstOrderVoucher: "cho đơn đầu tiên",
    hotDealStudentLabel: "Deal hot học sinh sinh viên",
    laptopPromoLabel: "Laptop ưu đãi khủng",
    tradeInTitle: "Thu cũ lên đời",
    iphoneTradeInLabel: "iPhone trợ giá đến 3 triệu",
    samsungTradeInLabel: "Samsung trợ giá đến 4 triệu",
    b2bTitle: "Khách hàng doanh nghiệp (B2B)",
    registerSBusinessLabel: "Đăng ký S-Business",
    policyPromoLabel: "Chính sách ưu đãi",
    newsletterTitle: "Nhận ưu đãi",
    newsletterDesc: "Đăng ký email để nhận voucher",
    newsletterVoucher: "cho đơn đầu tiên",
    accessoryTitle: "Sắm thêm phụ kiện chất lượng",
    accessoryApple: "Phụ kiện Apple",
    accessoryCable: "Cáp, sạc",
    accessoryBattery: "Pin sạc dự phòng",
    accessoryCase: "Ốp lưng - Bao da",
    accessoryScreen: "Dán màn hình",
    accessoryStorage: "Thẻ nhớ, USB",
    accessoryNetwork: "Thiết bị mạng",
    accessoryCamera: "Máy ảnh",
    accessoryKeyboard: "Chuột, bàn phím",
    accessoryBag: "Balo, túi xách",
    accessoryHub: "Hub chuyển đổi",
    accessoryPhone: "Phụ kiện điện thoại",
    backToHome: "Quay về trang chủ",
    accessoryLaptop: "Phụ kiện Laptop",
    gamingGear: "Gaming Gear, Playstation",
    sim4g5g: "Sim 4G - 5G",
    gimbal: "Gimbal",
    flycam: "Flycam",
    camera: "Camera",
    usedGoodsTitle: "Hàng cũ",
    usedPhone: "Điện thoại cũ",
    usedTablet: "Máy tính bảng cũ",
    usedMacbook: "MacBook cũ",
    usedLaptop: "Laptop cũ",
    usedHeadphone: "Tai nghe cũ",
    usedSpeaker: "Loa cũ",
    usedWatch: "Đồng hồ thông minh cũ",
    usedHome: "Đồ gia dụng cũ",
    usedAccessory: "Phụ kiện cũ",
    usedMonitor: "Màn hình cũ",
    usedTv: "Tivi cũ",
    usedCable: "Cáp sạc cũ",
    bannerSubOpenSale: "Mở bán ưu đãi khủng",
    bannerSubRegister: "Đăng ký nhận tin",
    bannerSubGift: "Mở bán tặng quà 15 triệu",
    bannerSubIphoneDeal: "Mở bán ưu đãi đến 2.65 triệu",
    bannerSubSony: "Tinh xảo chất âm. Chống ồn độc nhất.",
    bannerSubGalaxy: "Giá chỉ từ 10.79 triệu",
    bannerSubAirpods: "Đặt trước ngay",
    newsTitle1:
      "iPhone 16 Pro Max: Đánh giá chi tiết sau 1 tháng sử dụng thực tế",
    newsTitle2: "Top 5 laptop gaming tốt nhất 2025 đáng mua nhất hiện nay",
    newsTitle3: "Samsung Galaxy S25 Ultra ra mắt với AI mạnh mẽ chưa từng có",
    newsTitle4: "AirPods Pro 3 lộ diện thiết kế hoàn toàn mới, pin cải tiến",
    newsTitle5:
      "MacBook Air M4: Hiệu năng vượt trội, giá hợp lý hơn bao giờ hết",
    newsAlt1: "iPhone 16 Pro Max đánh giá chi tiết",
    newsAlt2: "Top laptop gaming 2025",
    newsAlt3: "Samsung Galaxy S25 Ultra AI",
    newsAlt4: "AirPods Pro 3 thiết kế mới",
    newsAlt5: "MacBook Air M4 hiệu năng",
  },
  footerNav: {
    laptopPc: "Laptop",
    smartphone: "Smart Phone",
    audioDevice: "Âm thanh",
    orderLookup: "Tra cứu đơn hàng",
    warrantyPolicy: "Chính sách bảo hành",
    faq: "FAQ",
    contactFooter: "Liên hệ",
    getOffers: "Nhận ưu đãi",
    emailPlaceholder: "Email của bạn",
    firstOrderVoucher: "cho đơn đầu tiên",
    signupOffer: "Đăng ký email để nhận voucher",
    terms: "Điều khoản",
    privacy: "Bảo mật",
    cookies: "Cookies",
    news: "Tin tức",
    contact: "Liên hệ",
  },
  news: {
    comingSoon: "Coming Soon",
    desc: "Nội dung tin tức sẽ sớm được cập nhật",
  },
  promotion: {
    comingSoon: "Coming Soon",
    desc: "Nội dung khuyến mãi sẽ sớm được cập nhật",
  },
  modalReview: {
    title: "Đánh giá & nhận xét",
    overallRating: "Đánh giá chung",
    byExperience: "Theo trải nghiệm",
    reviewLabel: "Nhận xét của bạn",
    reviewPlaceholder:
      "Xin mời chia sẻ một số cảm nhận về sản phẩm (nhập tối thiểu 15 kí tự)",
    sending: "ĐANG GỬI...",
    sendReview: "GỬI ĐÁNH GIÁ",
    ratingVeryBad: "Rất Tệ",
    ratingBad: "Tệ",
    ratingNormal: "Bình thường",
    ratingGood: "Tốt",
    ratingExcellent: "Tuyệt vời",
    expPerformance: "Hiệu năng",
    expBattery: "Thời lượng pin",
    expCamera: "Chất lượng camera",
    expDesign: "Thiết kế",
    expScreen: "Màn hình",
    expPerformanceLevels: "Yếu,Ổn,Khá,Mạnh,Siêu mạnh mẽ",
    expBatteryLevels: "Nhanh hết,Tạm ổn,Ổn,Trâu,Cực khủng",
    expCameraLevels: "Mờ,Ổn,Khá,Đẹp,Chụp đẹp, chuyên nghiệp",
    expDesignLevels: "Thô,Ổn,Hài hòa,Đẹp,Rất cao cấp",
    expScreenLevels: "Tạm,Ổn,Sáng,Đẹp,Rất sắc nét",
    productDefault: "Sản phẩm",
    authorName: "Khách hàng",
    authorPlaceholder: "Tên của bạn",
    errNoProduct: "Không tìm thấy sản phẩm để gửi đánh giá.",
    errNoRating: "Vui lòng chọn đánh giá chung.",
    errNoAuthorName: "Vui lòng nhập tên của bạn.",
    errMinChars: "Vui lòng nhập tối thiểu 15 kí tự.",
    errSubmitFailed: "Không thể gửi đánh giá lúc này.",
    success: "Gửi đánh giá thành công.",
  },
  checkout: {
    progressAria: "Tiến trình đặt hàng",
    backToCart: "Giỏ hàng",
    stepInfo: "Thông tin đặt hàng",
    deliveryAddress: "Địa chỉ giao hàng",
    noAddress: "Bạn chưa có địa chỉ giao hàng nào",
    addNewAddress: "Thêm địa chỉ mới",
    editAddress: "Chỉnh sửa địa chỉ",
    newAddress: "Thêm địa chỉ mới",
    receiverName: "Họ tên người nhận",
    receiverNamePlaceholder: "Nhập họ tên",
    phone: "Số điện thoại",
    phonePlaceholder: "0901 234 567",
    province: "Tỉnh/Thành phố",
    selectProvince: "Chọn Tỉnh/Thành",
    district: "Quận/Huyện",
    selectDistrict: "Chọn Quận/Huyện",
    ward: "Phường/Xã",
    selectWard: "Chọn Phường/Xã",
    detailAddress: "Địa chỉ chi tiết",
    detailAddressPlaceholder: "Số nhà, đường,...",
    addressLabel: "Nhãn địa chỉ",
    home: "Nhà riêng",
    office: "Văn phòng",
    setAsDefault: "Đặt làm địa chỉ mặc định",
    cancel: "Hủy",
    saveAddress: "Lưu địa chỉ",
    saving: "Đang lưu...",
    default: "Mặc định",
    setDefault: "Đặt mặc định",
    settingDefault: "Đang đặt...",
    edit: "Sửa",
    delete: "Xóa",
    searchAddress: "Tìm kiếm...",
    noResults: "Không có kết quả",
    addressAdded: "Đã thêm địa chỉ mới.",
    addressUpdated: "Đã cập nhật địa chỉ.",
    addressDeleted: "Đã xóa địa chỉ.",
    addressError: "Không thể lưu địa chỉ.",
    deleteError: "Không thể xóa địa chỉ.",
    confirmDeleteAddress: 'Xoá địa chỉ của "{name}"?',
    deliveryMethod: "Phương thức giao hàng",
    selectAddressFirst: "Vui lòng chọn địa chỉ giao hàng trước.",
    selectAddressForAccurateFee: "Chọn địa chỉ giao hàng để có phí ship chính xác.",
    deliveryTimeSlot: "Khung giờ giao hàng",
    free: "Miễn phí",
    voucher: "Mã giảm giá",
    enterVoucherCode: "Nhập mã giảm giá",
    apply: "Áp dụng",
    voucherApplied: "Đã áp dụng mã giảm giá.",
    voucherNotApplicable: "Mã giảm giá không áp dụng được.",
    expires: "HSD",
    use: "Dùng",
    applied: "Đã dùng",
    voucherSuggestion: "Đề xuất",
    voucherRejected: "Không thể áp dụng",
    voucherNotStackable: "Không áp dụng chung với voucher khác",
    freeship: "Freeship",
    savingsAmount: "Tiết kiệm {amount}",
    noVouchersAvailable: "Không có voucher khả dụng",
    applyingVoucher: "Đang kiểm tra...",
    applyVouchers: "Áp dụng ({count}) voucher",

    // Voucher history page
    voucherHistory: {
      title: "Lịch sử voucher",
      totalSaved: "Tổng tiết kiệm được",
      usedOn: "Sử dụng lúc",
      usedCount: "Đã dùng {count} lần",
      saved: "đã tiết kiệm",
      empty: "Bạn chưa sử dụng voucher nào.",
      retry: "Thử lại",
    },
    orderNote: "Ghi chú đơn hàng",
    optional: "Tùy chọn",
    orderNotePlaceholder: "Ví dụ: Giao giờ hành chính, gọi trước khi giao...",
    orderInsurance: "Bảo hiểm đơn hàng",
    insuranceFee: "29.000đ",
    insuranceTitle: "Bảo vệ toàn diện",
    insuranceCover1: "Rơi vỡ",
    insuranceCover2: "Ngập nước",
    insuranceFeeNote: "Phí: 29.000đ",
    addInsurance: "Thêm vào đơn",
    insuranceAdded: "Đã thêm bảo hiểm",
    deliverToOther: "Giao cho người khác",
    enableSecondary: "Giao cho người nhận thay thế",
    notifyOriginal: "Gửi thông báo SMS cho người đặt khi giao hàng",
    expressCheckout: "Đặt hàng nhanh với:",
    zaloPay: "ZaloPay",
    visa: "VISA",
    reviewOrder: "Xác nhận đơn hàng",
    change: "Thay đổi",
    back: "Quay lại",
    confirmOrder: "Xác nhận đặt hàng",
    orderSummary: "Đơn hàng",
    orderCount: "{count} sản phẩm",
    viewDetails: "Xem chi tiết",
    noItems: "Không có sản phẩm nào",
    subtotal: "Tạm tính",
    discount: "Giảm giá",
    deliveryFee: "Phí giao hàng",
    total: "TỔNG CỘNG",
    insurance: "Bảo hiểm",
    agreeTerms: "Tôi đồng ý với",
    terms: "Điều khoản",
    and: "và",
    shippingPolicy: "Chính sách giao hàng",
    termsUrl: "/terms",
    shippingPolicyUrl: "/shipping-policy",
    agreeRequired: "Vui lòng đồng ý với điều khoản.",
    placeOrder: "ĐẶT HÀNG",
    processing: "Đang xử lý...",
    submitError: "Không thể xử lý đơn hàng. Vui lòng thử lại.",
    itemCount: "{count} sản phẩm",
    securePayment: "Thanh toán an toàn",
    secureInfo: "Thông tin của bạn được bảo mật",
    trust1: "Miễn phí đổi trả trong 30 ngày",
    trust2: "Giao hàng nhanh toàn quốc",
    trust3: "Thanh toán an toàn (SSL 256-bit)",
    trust4: "Hỗ trợ 24/7",
  },
  payment: {
    title: "Phương thức thanh toán",
    momo: "Ví MoMo",
    vnpay: "VNPay",
    cod: "Thanh toán khi nhận hàng (COD)",
    momoDesc: "Thanh toán qua ví MoMo - Nhanh chóng và an toàn",
    vnpayDesc: "Thanh toán qua cổng VNPay - Hỗ trợ 40+ ngân hàng",
    codDesc: "Trả tiền mặt khi nhận được hàng",
    processing: "Đang xử lý thanh toán...",
    success: "Thanh toán thành công",
    failed: "Thanh toán thất bại",
    orderComplete: "Đơn hàng đã được đặt",
    transactionId: "Mã giao dịch",
    qrTitle: "Thanh toán {method}",
    qrScanHint: "Quét mã QR bằng ứng dụng {method}",
    qrWaiting: "Đang chờ thanh toán...",
    qrLoadError: "Không tải được mã QR",
    qrExpired: "Mã thanh toán đã hết hạn",
    qrExpiredHint: "Vui lòng tạo mã mới để tiếp tục",
    qrTip: "Mẹo: Mở app {method} trên điện thoại rồi quét mã QR trên màn hình",
    qrCancel: "Hủy thanh toán",
    popupOpen:
      "Cửa sổ thanh toán đã mở — trang này sẽ tự chuyển khi thanh toán hoàn tất",
    qrRetry: "Thử lại",
    mobileRedirecting: "Đang chuyển hướng đến ứng dụng...",
    proceedToPayment: "Tiến hành thanh toán",
    selectMethod: "Vui lòng chọn phương thức thanh toán",
    selectMethodFirst: "Chọn phương thức thanh toán để tiếp tục",
    paymentError: "Không thể tạo thanh toán. Vui lòng thử lại.",
    noPreparedOrder:
      "Không tìm thấy đơn hàng. Vui lòng quay lại bước đặt hàng.",
    confirmingPayment: "Đang xác nhận thanh toán...",
    confirmingDesc:
      "Vui lòng chờ trong giây lát trong khi chúng tôi xác nhận thanh toán với cổng thanh toán.",
    confirmSuccess: "Thanh toán thành công!",
    confirmFailed: "Thanh toán không thành công",
    confirmTimeout: "Không thể xác nhận thanh toán",
    confirmTimeoutDesc:
      "Chúng tôi không thể xác nhận thanh toán. Kết quả thanh toán có thể vẫn được xử lý.",
    pollingIndicator: "Đang kiểm tra trạng thái...",
    noPaymentInfo: "Không tìm thấy thông tin thanh toán",
    noPaymentInfoDesc: "Vui lòng thực hiện thanh toán để đặt hàng.",
    retryConfirm: "Kiểm tra lại",
    billItems: "Sản phẩm đã đặt",
    billSummary: "Tổng kết",
    billShipping: "Địa chỉ giao hàng",
    billPayment: "Thông tin thanh toán",
    viewOrders: "Xem đơn hàng",
    confirmRemoveVoucher: 'Gỡ mã voucher "{code}"?',
  },
  orderSuccess: {
    title: "Đặt hàng thành công",
    titleFailed: "Đặt hàng không thành công",
    orderPlaced: "Cảm ơn bạn! Đơn hàng đã được đặt thành công.",
    orderNumber: "Mã đơn hàng",
    verifying: "Đang xác minh thanh toán với máy chủ...",
    verifyFailed:
      "Không thể xác minh với máy chủ. Kết quả từ cổng thanh toán vẫn được hiển thị.",
    transactionDetails: "Chi tiết giao dịch",
    amount: "Số tiền",
    bank: "Ngân hàng",
    cardType: "Loại thẻ",
    transactionNo: "Mã giao dịch",
    transactionDate: "Thời gian",
    momoTransactionId: "Mã giao dịch MoMo",
    errorReason: "Lý do",
    transactionFailed: "Giao dịch thanh toán không thành công",
    codNote: "Bạn sẽ thanh toán khi nhận hàng (COD).",
    stepConfirm: "Chúng tôi sẽ liên hệ xác nhận đơn hàng qua điện thoại",
    stepDelivery: "Đơn hàng sẽ được giao trong 1-3 ngày làm việc",
    stepCod: "Nhận hàng và thanh toán khi nhận",
    nextSteps: "Các bước tiếp theo",
    backToCart: "Quay lại giỏ hàng",
    gotoHome: "Về trang chủ",
  },

  profile: {
    // === Common ===
    common: {
      update: "Cập nhật",
      add: "Thêm",
      edit: "Sửa",
      delete: "Xóa",
      close: "Đóng",
      cancel: "Hủy",
      save: "Lưu",
      confirm: "Xác nhận",
      success: "Thành công",
      error: "Lỗi",
      loading: "Đang tải...",
      noData: "Không có dữ liệu",
      home: "Trang chủ",
      message: "Nhắn tin",
      detail: "Chi tiết",
      seeAll: "Xem tất cả",
      noResults: "Không có kết quả",
    },

    // === ProfileLayout (component) ===
    layout: {
      memberCard: {
        rank: "Hạng",
        spent: "Đã mua",
        renewHint: "Cập nhật lại sau {date}",
        progressHint: "Cần chi tiêu thêm {amount} để lên hạng {rank}",
        progressHintMobile: "Cần chi tiêu thêm {amount} để lên hạng {rank}",
        totalSpent: "Tổng tiền tích lũy",
        fromDate: "Từ 01/01/2025",
        totalOrders: "Tổng số đơn hàng",
        totalOrdersSuffix: " đã mua",
        channelNote: "Tổng tiền và số đơn hàng được tính chung từ IrusGear.",
      },
      addPhone: "Nhập số điện thoại",
      phonePlaceholder: "Nhập số điện thoại",
      avatar: {
        update: "Cập nhật ảnh đại diện",
        updateSuccess: "Cập nhật ảnh đại diện thành công",
        updateFailed: "Cập nhật ảnh đại diện thất bại",
        errors: {
          invalidFile: "Ảnh đại diện không hợp lệ.",
          unsupportedType: "Ảnh đại diện chỉ hỗ trợ JPG, PNG hoặc WEBP.",
          emptyFile: "Ảnh đại diện không có dung lượng hợp lệ.",
          tooLarge: "Ảnh đại diện không được vượt quá 3MB.",
          createTargetFailed: "Không tạo được URL tải ảnh đại diện.",
          uploadHttpFailed: "Tải ảnh đại diện thất bại với HTTP {status}.",
          networkFailed: "Tải ảnh đại diện thất bại do lỗi mạng.",
          aborted: "Tải ảnh đại diện đã bị hủy.",
        },
      },
      stats: {
        orders: "Đơn hàng",
        pending: "Chờ xử lý",
        warranty: "Bảo hành",
        tradein: "Thu cũ",
      },
      quickActions: {
        profileInfo: "Thông tin tài khoản",
        addressBook: "Sổ địa chỉ",
        changePassword: "Đổi mật khẩu",
        linkedAccounts: "Tài khoản liên kết",
        memberRank: "Hạng thành viên",
        discountCode: "Mã giảm giá",
        orderHistory: "Lịch sử mua hàng",
      },
      sidebar: {
        title: "Tài khoản",
        overview: "Tổng quan",
        orders: "Đơn hàng",
        warranty: "Bảo hành",
        tradein: "Thu cũ đổi mới",
        promotion: "Khuyến mãi",
        userInfo: "Thông tin cá nhân",
        policy: "Chính sách",
        terms: "Điều khoản",
        logout: "Đăng xuất",
      },
      appCta: {
        title: "Mua sắm dễ dàng - Ưu đãi ngập tràn cùng IrusGear",
        download: "Tải ứng dụng IrusGear",
      },
    },

    // === pages/profile/index.vue ===
    dashboard: {
      noticeAddress: "Thêm địa chỉ để đặt đơn hàng nhanh hơn.",
      addAddress: "Thêm địa chỉ",
      closeNotice: "Đóng",
      recentOrdersTitle: "Đơn hàng gần đây",
      yourOffersTitle: "Ưu đãi của bạn",
      favoritesTitle: "Sản phẩm yêu thích",
      seeAllOrders: "Xem tất cả",
      seeAllOffers: "Xem tất cả",
      seeAllFavorites: "Xem tất cả",
      noOrders:
        "Bạn chưa có đơn hàng nào gần đây? Hãy bắt đầu mua sắm ngay nào!",
      noOffers: "Bạn chưa có ưu đãi nào.",
      noFavorites: "Bạn chưa có sản phẩm yêu thích nào.",
      addFavoriteProducts: "Thêm sản phẩm yêu thích",
      shopNow: "Mua sắm ngay",
      seeProducts: "Xem sản phẩm",
      copyCode: "Sao chép mã",
      expires: "Hết hạn",
      used: "Đã sử dụng",
      addressesTitle: "Địa chỉ của bạn",
      noAddresses: "Bạn chưa có địa chỉ nào.",
      editAddress: "Sửa địa chỉ",
      default: "Mặc định",
    },

    // === pages/orders/index.vue ===
    orders: {
      title: "Đơn hàng",
      tabs: {
        all: "Tất cả",
        pending: "Chờ xác nhận",
        processing: "Đang xử lý",
        shipping: "Đang giao",
        delivered: "Đã giao",
        cancelled: "Đã hủy",
      },
      empty: "Bạn chưa có đơn hàng nào",
      goHome: "Trang chủ",
      filterDate: "Lọc theo ngày",
      orderId: "Mã đơn #",
      total: "Tổng cộng:",
      contact: "Nhắn tin",
      detail: "Chi tiết",
      status: {
        pending: "Chờ xác nhận",
        processing: "Đang xử lý",
        shipped: "Đã giao cho đơn vị vận chuyển",
        delivering: "Đang giao hàng",
        delivered: "Đã giao",
        cancelled: "Đã hủy",
      },
    },

    // === pages/orders/[id].vue ===
    orderDetail: {
      title: "Chi tiết đơn hàng",
      orderNumber: "Đơn hàng",
      timeline: "Lịch sử trạng thái",
      items: "Sản phẩm đã đặt",
      summary: "Tóm tắt đơn hàng",
      shipping: "Thông tin giao hàng",
      payment: "Thông tin thanh toán",
      note: "Ghi chú",
      deliveryMethod: "Phương thức giao hàng",
      deliveryStandard: "Giao tiêu chuẩn",
      deliveryExpress: "Giao hỏa tốc",
      deliverySameDay: "Giao trong ngày",
      deliveryPickup: "Nhận tại cửa hàng",
      tracking: "Mã vận đơn",
      estimatedDelivery: "Dự kiến giao",
      backToList: "Quay lại danh sách",
      cancelOrder: "Hủy đơn hàng",
      reorder: "Đặt lại",
      confirmCancel: "Bạn có chắc muốn hủy đơn hàng này?",
      cancelSuccess: "Đã gửi yêu cầu hủy đơn hàng",
      cancelError: "Không thể hủy đơn hàng",
      fetchError: "Không thể tải thông tin đơn hàng",
      notFound: "Không tìm thấy đơn hàng",
      notFoundDesc: "Đơn hàng có thể đã bị xóa hoặc bạn không có quyền xem.",
      proofOfDelivery: {
        title: "Bằng chứng giao hàng",
        deliveredAt: "Thời gian giao",
        recipient: "Người nhận",
        viewLarger: "Mở ảnh lớn",
      },
    },

    // === pages/warranty/index.vue ===
    warranty: {
      title: "Bảo hành",
      tabs: {
        all: "Tất cả",
        received: "Tiếp nhận",
        coordinating: "Đang phối hợp",
        repairing: "Đang sửa chữa",
        done: "Hoàn tất",
        returned: "Đã trả khách",
      },
      searchPlaceholder: "Tìm kiếm theo mã đơn, tên sản phẩm...",
      empty: "Bạn chưa có đơn bảo hành nào",
      labels: {
        serial: "Số serial",
        imei: "IMEI",
        purchaseDate: "Ngày mua",
        warrantyEnd: "Hạn bảo hành",
        serviceCenter: "Trung tâm bảo hành",
      },
    },

    // === pages/tradein/index.vue ===
    tradein: {
      historyTitle: "Lịch sử thu cũ",
      empty: "Bạn chưa có phiếu thu cũ nào",
      oldDevice: "Thiết bị cũ",
      newDevice: "Thiết bị mới",
      oldValue: "Giá trị thu cũ:",
      newPrice: "Giá máy mới:",
      topUp: "Thanh toán thêm:",
      contact: "Nhắn tin",
      detail: "Chi tiết",
      filterDate: "Lọc theo ngày",
      status: {
        pending: "Chờ xử lý",
        done: "Hoàn tất",
        cancelled: "Đã hủy",
      },
    },

    // === pages/promotion/index.vue ===
    promotion: {
      yourOffers: "Ưu đãi của bạn",
      noOffers: "Bạn đang chưa có ưu đãi nào",
      rankTitle: "Hạng thành viên",
      shoppingBenefits: "Quyền lợi mua hàng",
      servicePolicy: "Chính sách phục vụ",
      upgradeConditions: "Điều kiện thăng cấp",
      rankStatus: {
        current: "Hạng hiện tại",
        locked: "Đã khóa",
      },
      unlocked: "Đã mở khóa",
      notUnlocked: "Chưa mở khóa",
      spent: "Đã mua",
      renewHint: "Hạng thành viên được cập nhật lại sau",
      needSpend: "Cần chi tiêu thêm",
      toRank: "để lên hạng",
      ranks: {
        snull: "S-NULL",
        snew: "S-NEW",
        smem: "S-MEM",
        svip: "S-VIP",
        s_null: "S-NULL",
        s_new: "S-NEW",
        s_mem: "S-MEM",
        s_vip: "S-VIP",
      },
      student: "Học sinh - Sinh viên",
    },

    // === pages/user-info/index.vue ===
    userInfo: {
      alertGender: "Vui lòng cập nhật giới tính để có trải nghiệm tốt hơn.",
      updateGender: "Cập nhật",
      personalInfo: "Thông tin cá nhân",
      fullName: "Họ và tên:",
      gender: "Giới tính:",
      birthday: "Ngày sinh:",
      phone: "Số điện thoại:",
      email: "Email:",
      address: "Địa chỉ mặc định:",
      addressBook: "Sổ địa chỉ",
      addAddress: "Thêm địa chỉ",
      noAddress: "Bạn chưa có địa chỉ nào được tạo.",
      password: "Mật khẩu",
      changePassword: "Thay đổi mật khẩu",
      lastUpdated: "Cập nhật lần cuối lúc:",
      linkedAccounts: "Tài khoản liên kết",
      link: "Liên kết",
      unlink: "Hủy liên kết",
      linked: "Đã liên kết",
      linkedAt: "Liên kết lúc:",
      update: "Cập nhật",
      genderOptions: {
        male: "Nam",
        female: "Nữ",
        other: "Khác",
      },
    },

    // === pages/policy/index.vue ===
    policy: {
      pageTitle: "Chính sách bảo hành - IrusGear",
      sidebarTitle: "Bảo hành",

      // Sidebar nav
      nav: {
        return30days: "I. Đổi mới 30 ngày miễn phí",
        standard: "II. Bảo hành tiêu chuẩn",
        components: "III. Linh kiện máy tính",
        vip: "IV. Bảo hành 1 đổi 1 VIP",
        accidental: "V. Bảo hành rơi vỡ, ngấm nước",
        s24plus: "VI. Bảo hành mở rộng S24+",
      },

      // Section I - Return 30 days
      return: {
        title: "I. Đổi mới 30 ngày miễn phí",
        desc: "Áp dụng khi máy có lỗi phần cứng từ phía nhà sản xuất.",
        tableHeaders: {
          product: "Sản phẩm",
          freeExchange: "Đổi mới miễn phí",
          returnPolicy: "Quy định nhập lại, trả lại (thu cũ)",
        },
        rows: [
          {
            product: "Điện thoại, Máy tính bảng, Macbook, Apple Watch",
            freeExchange: "30 ngày",
            returnPolicy:
              "Trong 30 ngày đầu: trừ phí 20% trên giá hiện tại. Sau 30 ngày: Nhập lại theo giá thỏa thuận.",
          },
          {
            product: "Samsung Watch",
            freeExchange: "30 ngày",
            returnPolicy:
              "Trong 30 ngày đầu: trừ phí 30% trên giá hiện tại. Sau 30 ngày: Nhập lại theo giá thỏa thuận.",
          },
          {
            product: "Laptop",
            freeExchange: "30 ngày",
            returnPolicy:
              "Trong 30 ngày đầu: trừ phí 20% trên giá hiện tại. Sau 30 ngày: Nhập lại theo giá thỏa thuận.",
          },
          {
            product: "Màn hình máy tính",
            freeExchange: "15 ngày",
            returnPolicy:
              "Trong 15 ngày đầu: trừ phí 20% trên giá hiện tại. Sau 15 ngày: Nhập lại theo giá thỏa thuận.",
          },
          {
            product: "Loa - Tai nghe cao cấp",
            freeExchange: "15 ngày",
            returnPolicy:
              "Trong 30 ngày đầu: trừ phí 40% trên giá mua ban đầu. Từ 31-60 ngày: trừ phí 50% trên giá mua ban đầu.",
          },
          {
            product: "Hàng cũ",
            freeExchange: "30 ngày",
            returnPolicy:
              "Trong 30 ngày đầu: trừ phí 15% trên giá hiện tại. Sau 30 ngày: Nhập lại theo giá thỏa thuận.",
          },
          {
            product: "Tivi / Đồ gia dụng",
            freeExchange: "30 ngày / 15 ngày",
            returnPolicy: "Không áp dụng nhập lại.",
          },
          {
            product: "Phụ kiện > 1 triệu",
            freeExchange: "15 ngày",
            returnPolicy:
              "Không áp dụng nhập lại. (Riêng Airpods: trừ phí 20% trong 30 ngày đầu.)",
          },
        ],
        conditionsTitle: "Điều kiện đổi trả",
        conditions: [
          {
            text: "Máy mới: Như mới, không trầy xước, không dán decal, hình trang trí.",
          },
          { text: "Máy cũ: Có tình trạng sản phẩm như lúc mới mua." },
          {
            text: "Hộp: Như mới, không móp méo, rách, viết vẽ, quấn băng dính... Số Serial/IMEI trên hộp phải trùng với thân máy.",
          },
          {
            text: "Phụ kiện và quà tặng: Còn đầy đủ, nguyên vẹn, không móp méo hoặc hư hại trong quá trình sử dụng.",
          },
          {
            text: "Tài khoản: Máy đã được đăng xuất khỏi tất cả các tài khoản (iCloud, Google, Mi Account...).",
          },
        ],
        screenDefectTitle: "Quy định về lỗi màn hình",
        screenDefects: [
          { text: "Màn hình có từ 3 điểm chết trở lên." },
          {
            text: "Hoặc 1 điểm chết có kích thước lớn hơn 1mm (điện thoại) và 5 điểm chết trở lên (laptop, màn hình rời).",
          },
        ],
      },

      // Section II - Standard warranty
      standard: {
        title: "II. Bảo hành tiêu chuẩn",
        phoneLaptopTitle: "Điện thoại, Laptop",
        phoneLaptopTableHeaders: {
          product: "Sản phẩm",
          warrantyTime: "Thời gian bảo hành",
          warrantyRights: "Quyền lợi bảo hành",
          warrantyAddress: "Địa chỉ bảo hành",
        },
        phoneLaptopRows: [
          {
            product: "Hàng mới",
            warrantyTime: "12 tháng (hoặc dài hơn theo quy định hãng)",
            warrantyRights: "Quyền lợi bảo hành của hãng",
            warrantyAddress: "TTBH chính hãng",
          },
          {
            product: "Hàng đã kích hoạt bảo hành chính hãng",
            warrantyTime:
              "12 tháng = Thời gian BH còn lại tại hãng + BH tại IrusGear",
            warrantyRights:
              "Theo quyền lợi BH của hãng trong thời gian được hãng BH; Sửa chữa, thay thế linh kiện trong thời gian BH còn lại tại IrusGear",
            warrantyAddress: "TTBH chính hãng & IrusGear / Điện Thoại Vui",
          },
          {
            product: "Hàng cũ",
            warrantyTime: "6 tháng",
            warrantyRights:
              "Sửa chữa, thay thế linh kiện, bao gồm cả nguồn và màn hình.",
            warrantyAddress: "IrusGear / Điện Thoại Vui",
          },
        ],
        phoneLaptopNote:
          "Trong thời gian đợi bảo hành – sửa chữa, khách hàng sẽ được hỗ trợ miễn phí một điện thoại khác để sử dụng tạm thời. Khách hàng trả lại máy đã mượn sau khi nhận lại máy của mình.",
        accessoryTitle: "Phụ kiện đi kèm máy",
        accessoryTableHeaders: {
          brand: "Hãng",
          warrantyTime: "Thời gian BH",
          rights: "Quyền lợi",
          address: "Địa chỉ",
        },
        accessoryRows: [
          {
            brand: "Vsmart",
            warrantyTime: "6 tháng",
            rights: "1 đổi tất cả các lỗi",
            address: "TTBH chính hãng",
          },
          {
            brand: "Asus",
            warrantyTime: "6 tháng",
            rights: "1 đổi tất cả các lỗi",
            address: "TTBH chính hãng",
          },
          {
            brand: "Nokia",
            warrantyTime: "6 tháng",
            rights: "1 đổi tất cả các lỗi",
            address: "TTBH chính hãng",
          },
        ],
      },

      // Section III - Computer components
      components: {
        title: "III. Bảo hành linh kiện máy tính",
        tableHeaders: {
          product: "Sản phẩm",
          warrantyTime: "Thời gian BH",
          warrantyRights: "Quyền lợi BH (lỗi nhà sản xuất)",
          location: "Địa điểm",
        },
        rows: [
          {
            product: "Bộ vi xử lý – CPU",
            warrantyTime: "36 tháng",
            rights: "Đổi mới sau khi có xác nhận lỗi từ hãng",
            location: "TTBH hãng",
          },
          {
            product: "Vỏ máy tính – Case",
            warrantyTime: "12 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
          {
            product: "Bo mạch chủ – Mainboard",
            warrantyTime: "36 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
          {
            product: "Nguồn – PSU",
            warrantyTime: "36 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
          {
            product: "RAM PC/Laptop",
            warrantyTime: "36 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
          {
            product: "Ổ cứng – SSD/HDD",
            warrantyTime: "36 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
          {
            product: "Card màn hình – VGA",
            warrantyTime: "36 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
          {
            product: "Tản nhiệt – Fan/Cooling",
            warrantyTime: "12 tháng",
            rights: "Sửa chữa hoặc đổi tương đương",
            location: "TTBH hãng",
          },
        ],
        conditionsTitle: "Điều kiện bảo hành",
        eligibleTitle: "Đủ điều kiện",
        notEligibleTitle: "Không đủ điều kiện (Từ chối bảo hành)",
        eligible: [
          {
            text: "Hư hỏng được xác định do lỗi kỹ thuật hoặc lỗi của nhà sản xuất.",
          },
          {
            text: "Sản phẩm còn trong thời hạn bảo hành, có tem bảo hành của nhà phân phối hoặc hãng sản xuất.",
          },
          {
            text: "Với sản phẩm cần số serial hoặc hộp, sản phẩm phải còn đầy đủ những thứ này.",
          },
          {
            text: "Sản phẩm không trầy xước, cấn móp, bể, vỡ, biến dạng so với ban đầu.",
          },
          {
            text: "Dùng đúng nguồn điện, không bị mối mọt, không nằm trong môi trường cháy nổ, phòng tủ không bị rỉ oxy hóa do đặt trong môi trường ẩm ướt.",
          },
          {
            text: "Chưa có sự can thiệp vào phần cứng (tự ý tháo dỡ, sửa chữa...).",
          },
          {
            text: "CPU Intel: Khách hàng cần giữ hộp nhựa đựng sản phẩm trong suốt thời gian bảo hành.",
          },
          {
            text: "Ổ cứng SSD: Bảo hành theo số tháng quy định hoặc theo chỉ số TBW của nhà sản xuất, tùy điều kiện nào đến trước.",
          },
        ],
        notEligible: [
          { text: "Hết thời hạn bảo hành." },
          { text: "CPU bị mẻ, nứt hoặc cong chân." },
          {
            text: "Không có tem bảo hành của Công ty/nhà phân phối/hãng hoặc tem bảo hành bị rách, tẩy xóa, sửa chống, không đọc được, bong tróc.",
          },
          {
            text: "Tác động vật lý làm trầy xước, cong vênh, rạn nứt, bể, vỡ trong quá trình sử dụng.",
          },
          {
            text: "Tự ý tháo mở, sửa chữa, thay đổi cấu trúc sản phẩm bên trong.",
          },
          {
            text: "Hư hỏng, cháy, nổ do sử dụng sai mục đích, tự ý tháo lắp đặt không tuân theo hướng dẫn.",
          },
          {
            text: "Hư hỏng do côn trùng xâm nhập (chuột, gián, kiến, mối...).",
          },
          {
            text: "Hư hỏng do thiên tai, hỏa hoạn, lũ lụt, sét đánh, rỉ sét, hao mòn do môi trường.",
          },
          {
            text: "Sản phẩm sử dụng với mục đích sản xuất, khai thác tiền điện tử (đào coin).",
          },
        ],
      },

      // Section IV - VIP 1-to-1
      vip: {
        title: "IV. Bảo hành 1 đổi 1 VIP",
        desc: "Áp dụng cho điện thoại, máy tính bảng (mới/cũ), tai nghe cao cấp mới, đồng hồ thông minh (không bao gồm Apple/Samsung mới). Thời gian: 06 – 12 tháng tùy dòng máy.",
        tableHeaders: {
          scope: "Phạm vi bảo hành",
          standard: "Bảo hành tiêu chuẩn",
          vip: "Bảo hành 1 đổi 1 – VIP",
        },
        rows: [
          {
            scope: "Mainboard (nguồn, ổ cứng)",
            standard: "Sửa chữa / Thay thế linh kiện",
            vip: "1 đổi 1 máy tương đương",
          },
          {
            scope: "Màn hình, cảm ứng (điểm chết >3, điểm đen >1mm)",
            standard: "Sửa chữa / Thay thế",
            vip: "1 đổi 1 máy tương đương",
          },
          {
            scope: "Linh kiện khác (camera, loa, wifi, chân sạc...)",
            standard: "Sửa chữa",
            vip: "1 đổi 1 máy tương đương",
          },
          {
            scope: "Pin và phím vật lý",
            standard: "Bảo hành 6 tháng (iPhone cũ: 3 tháng)",
            vip: "1 đổi 1 trong 12 tháng",
          },
        ],
        batteryNote:
          "* Pin được xác định lỗi khi hao trên 20% sau 1 giờ xem Youtube (wifi, độ sáng/âm lượng 50%) hoặc bị phồng.",
        processingTimeTitle: "Thời gian xử lý",
        processingTime: [
          {
            text: "Điện thoại, máy tính bảng, laptop (trừ Apple): 03 – 05 ngày làm việc.",
          },
          {
            text: "Sản phẩm Apple: 03 – 05 ngày (tại CareS) hoặc 07 – 10 ngày (tại TTBH ủy quyền).",
          },
          { text: "Phụ kiện và Tivi: 07 – 10 ngày làm việc." },
          { text: "Tổng thời gian thông thường: 07 – 14 ngày làm việc." },
        ],
        upgradeTitle: "Đổi sang máy giá trị cao hơn",
        upgradeFormula: "Giá nhập lại = Giá bán hiện tại của máy cũ × 90%",
        outOfStockTitle: "Trường hợp hết hàng để đổi",
        outOfStockDesc:
          "Nếu quá 07 ngày mà không có máy đổi, cửa hàng sẽ nhập lại máy theo công thức:",
        outOfStockFormula:
          "Giá trị nhập lại = Giá mua ban đầu × [100% − (5% × số tháng sử dụng)]",
        outOfStockNote:
          "* Thiếu hộp/phụ kiện sẽ trừ phí thêm khoảng 5% giá trị máy.",
        privilegeTitle: "Đặc quyền",
        privileges: [
          {
            text: "Khách hàng có thể chuyển nhượng quyền sở hữu máy và gói bảo hành VIP trong thời gian bảo hành.",
          },
          {
            text: "Được tặng thêm +5% giá trị thu mua khi có nhu cầu lên đời máy mới.",
          },
        ],
      },

      // Section V - Accidental damage
      accidental: {
        title: "V. Bảo hành rơi vỡ, ngấm nước",
        desc: "Áp dụng cho điện thoại, máy tính bảng mới/cũ (có giới hạn đối tượng sản phẩm áp dụng). Thời gian bảo hành: 12 tháng.",
        conditionsTitle: "Điều kiện bảo hành",
        conditions: [
          {
            text: "Sản phẩm bị tác động của ngoại lực, gây vỡ hoặc bị ngấm nước, ngấm các chất lỏng khác dẫn đến sản phẩm không hoạt động bình thường.",
          },
          {
            text: "Sản phẩm phải còn trong tình trạng có thể xác định được mã số IMEI/Serial định danh sản phẩm. Trường hợp không thể xác định được, IrusGear từ chối bảo hành.",
          },
        ],
        rightsTitle: "Quyền lợi bảo hành",
        rights: [
          {
            text: "Sản phẩm được sửa chữa/thay thế linh kiện tại các TTBH ủy quyền.",
          },
          {
            text: "Chi phí dịch vụ: Khách hàng chịu phí 10% chi phí sửa chữa/thay thế linh kiện.",
          },
          {
            text: "Giới hạn chi phí: Tổng chi phí cho các lần sửa chữa không vượt quá giá niêm yết của sản phẩm tại thời điểm mua. Nếu vượt quá, khách hàng chịu phần chênh lệch.",
          },
        ],
        cannotRepairTitle: "Trường hợp không thể sửa chữa",
        cannotRepair: [
          {
            text: "IrusGear sẽ đổi cho khách hàng một sản phẩm cũ có chất lượng tương đương.",
          },
          {
            text: "Khách hàng chịu phí dịch vụ 10% giá trị của sản phẩm được đổi.",
          },
          {
            text: "Sau khi đổi, gói bảo hành này hết hiệu lực và sản phẩm được đổi sẽ được bảo hành 06 tháng theo chính sách của IrusGear.",
          },
          {
            text: "Chỉ áp dụng đổi máy, không đổi hộp và phụ kiện kèm theo (sạc, cáp...).",
          },
        ],
        extraPrivilegeTitle: "Đặc quyền thêm",
        extraPrivilege:
          "Khách hàng sử dụng dịch vụ này có đặc quyền +3% tổng giá trị máy thu cũ khi lên đời trong thời gian bảo hành của thiết bị.",
      },

      // Section VI - S24+
      s24plus: {
        title: "VI. Bảo hành mở rộng S24+",
        desc: "Áp dụng cho Macbook, Điện thoại. Thời gian tổng cộng: 24 – 36 tháng (đã bao gồm 12 tháng bảo hành tiêu chuẩn từ nhà sản xuất). Gói bảo hành có hiệu lực ngay sau khi thời hạn bảo hành của nhà sản xuất kết thúc.",
        rightsTitle: "Quyền lợi bảo hành",
        rights: [
          {
            text: "Sửa chữa miễn phí 100%: Miễn phí chi phí sửa chữa và thay thế linh kiện nếu sản phẩm gặp lỗi từ nhà sản xuất.",
          },
          {
            text: "Thời gian xử lý: 07 – 14 ngày làm việc. Riêng Macbook có thể kéo dài 03 tuần đến 01 tháng.",
          },
          {
            text: "Chuyển nhượng: Khách hàng có thể chuyển nhượng quyền sở hữu sản phẩm kèm gói bảo hành S24+ cho người khác.",
          },
          {
            text: "Đặc quyền lên đời: Được cộng thêm +3% tổng giá trị máy thu cũ khi thu cũ đổi mới.",
          },
        ],
        cannotRepairTitle: "Không thể sửa chữa được",
        cannotRepairDesc:
          "Nếu máy bị lỗi nhà sản xuất nhưng không thể sửa chữa, IrusGear giải quyết theo thứ tự:",
        cannotRepairSteps: [
          {
            text: "Bước 1: Đổi sản phẩm tương đương cùng thương hiệu, dòng sản phẩm, có chất lượng tương đương tại thời điểm bảo hành.",
          },
          {
            text: "Bước 2: Nếu không có sản phẩm để đổi, nhập lại (hoàn tiền) theo công thức:",
          },
        ],
        cannotRepairFormula:
          "Giá nhập lại = Giá mua sản phẩm × (100% − 3% × Số tháng đã sử dụng)",
        notCoveredTitle: "Các trường hợp KHÔNG được bảo hành S24+",
        notCovered: [
          { text: "Các hư hỏng do người dùng gây ra như: rơi vỡ, vào nước..." },
          { text: "Các vấn đề liên quan đến pin và phím vật lý." },
        ],
        exampleTitle: "Ví dụ minh họa",
        exampleContent:
          "Nếu bạn mua tai nghe giá 4.800.000đ kèm gói S24+, sau 15 tháng sử dụng nếu bị lỗi nhà sản xuất mà không sửa được:",
        exampleBullets: [
          { text: "Bạn có thể được đổi một tai nghe cũ tương đương." },
          {
            text: "Hoặc được nhập lại với giá 2.640.000đ (tức 4.800.000 × 55%) để đổi sang sản phẩm khác hoặc nhận lại tiền.",
          },
        ],
      },

      // Shared notice
      notice: {
        dataLoss:
          "Lưu ý về dữ liệu: Khách hàng vui lòng tự sao lưu dữ liệu. Cửa hàng không chịu trách nhiệm về việc mất dữ liệu trong mọi trường hợp.",
        dataLossAlt:
          "Lưu ý về dữ liệu: Quý khách vui lòng chủ động sao lưu dữ liệu cá nhân trước khi gửi bảo hành. IrusGear và TTBH không chịu trách nhiệm về mất mát dữ liệu trong mọi trường hợp.",
        important:
          "Lưu ý quan trọng: IrusGear không chịu trách nhiệm về việc mất dữ liệu trong mọi trường hợp. Khách hàng nên chủ động sao lưu dữ liệu trước khi mang đi bảo hành.",
        importantAlt:
          "Lưu ý quan trọng: Khách hàng nên chủ động sao lưu dữ liệu cá nhân trước khi mang máy đi bảo hành vì cửa hàng không chịu trách nhiệm về mất mát dữ liệu.",
      },
    },

    // === pages/tos/index.vue ===
    tos: {
      pageTitle: "Quy chế hoạt động - IrusGear",
      sidebarTitle: "Quy chế",

      nav: {
        general: "I. Quy định chung",
        transaction: "II. Quy trình giao dịch",
        warranty: "III. Chính sách bảo hành sản phẩm",
        cancellation: "IV. Chính sách hủy đơn, đổi trả",
        privacyIrusgear: "V. Bảo mật thông tin IrusGear",
      },

      // I - General
      general: {
        title: "I. Quy định chung",
        principlesTitle: "1. Nguyên tắc chung",
        principles: [
          {
            text: 'Website thương mại điện tử IrusGear.com.vn là sở hữu của Công ty TNHH Thương mại và Dịch vụ Kỹ thuật Diệu Phúc ("Công ty") do Công ty phát triển, hoạt động và vận hành với tên miền giao dịch là www.irusgear.com.vn (sau đây gọi là Website). Đối tượng phục vụ là tất cả các khách hàng có nhu cầu mua hàng, đặt hàng thông qua Website.',
          },
          {
            text: "Sản phẩm được kinh doanh tại IrusGear.com.vn phải đáp ứng đầy đủ các quy định của nhà nước về nguồn gốc, xuất xứ hàng hóa, chất lượng sản phẩm.",
          },
          {
            text: "Hoạt động mua bán tại IrusGear.com.vn phải được thực hiện công khai, minh bạch, đảm bảo quyền lợi của người tiêu dùng.",
          },
        ],
        definitionsTitle: "2. Định nghĩa",
        definitions: [
          { text: "Người bán: là IrusGear." },
          {
            text: "Người mua: là mọi công dân sinh sống trong lãnh thổ Việt Nam.",
          },
          {
            text: "Thành viên: là bao gồm cả người mua và người tham gia tham khảo thông tin, thảo luận tại Website.",
          },
          {
            text: "Thành viên khi tham gia Website phải tự tìm hiểu trách nhiệm pháp lý của mình đối với luật pháp hiện hành của Việt Nam và cam kết thực hiện đúng những nội dung trong Quy chế này.",
          },
        ],
      },

      // II - Transaction
      transaction: {
        title: "II. Quy trình giao dịch",
        purchaseTitle: "1. Quy trình mua hàng trên Website",
        purchase: [
          {
            text: 'Bước 1: Khách hàng tìm kiếm sản phẩm, đọc và đồng ý với các quy định trong "Quy chế hoạt động website cung cấp dịch vụ TMĐT IrusGear.com.vn".',
          },
          {
            text: 'Bước 2: Xem giá và thông tin chi tiết sản phẩm. Nếu đồng ý mua, tùy theo nhu cầu và sản phẩm cụ thể, bấm một trong các nút: "Mua ngay (giao hàng tận nơi hoặc nhận tại cửa hàng)", "Trả góp (duyệt qua điện thoại)", hoặc "Trả góp qua thẻ (Visa, Master, JCB)".',
          },
          {
            text: "Bước 3: Khách hàng nhập thông tin đặt hàng theo hướng dẫn tại mỗi mục mua hàng đã chọn ở trên. Nếu có mã khuyến mãi, nhập trực tiếp vào phiếu đặt hàng.",
          },
          {
            text: "Bước 4: Sau khi nhận được đơn đặt hàng của khách, IrusGear sẽ liên hệ qua số điện thoại đã cung cấp qua tổng đài 1800.2063 để xác nhận thông tin đơn hàng.",
          },
          {
            text: "Bước 5: IrusGear giao hàng tận nơi hoặc khách hàng đến các cửa hàng IrusGear toàn quốc để nhận hàng.",
          },
        ],
        deliveryTitle: "2. Quy trình giao hàng và vận chuyển",
        deliveryDesc:
          "IrusGear miễn phí giao hàng toàn quốc. Sau khi tiếp nhận đơn đặt hàng của người mua và xác nhận thông tin mua hàng qua điện thoại, IrusGear sẽ tiến hành giao hàng theo yêu cầu của khách.",
        noticeImportant:
          "Lưu ý quan trọng: Với giao dịch có giá trị từ 10 triệu đồng trở lên, khách hàng vui lòng xuất trình THẺ GỐC và CCCD/CMND GỐC của CHỦ THẺ để IrusGear đối chiếu trước khi giao hàng nhằm phòng tránh trường hợp gian lận. Trường hợp người nhận hàng không phải là CHỦ THẺ hoặc phát hiện có vấn đề về giao dịch, IrusGear có quyền hủy giao dịch và hoàn tiền cho CHỦ THẺ. Thời gian xử lý hoàn tiền từ 7 – 14 ngày (không tính thứ 7, CN).",
      },

      // III - Warranty policy
      warranty: {
        title: "III. Chính sách bảo hành sản phẩm",
        return30Title: "1. Chính sách đổi mới 30 ngày miễn phí",
        return30Desc:
          "Khách hàng được đổi mới miễn phí 1 đổi 1 hoặc hoàn tiền 100% trong 30 ngày đầu tiên đối với lỗi phần cứng từ nhà sản xuất.",
        returnTableHeaders: {
          productGroup: "Nhóm sản phẩm",
          exchangeTime: "Thời gian đổi mới",
          returnPolicy: "Quy định nhập lại / Hoàn tiền",
        },
        returnRows: [
          {
            productGroup: "Điện thoại, Máy tính bảng, Macbook, Apple Watch",
            exchangeTime: "30 ngày",
            returnPolicy:
              "Trong 30 ngày: hoàn tiền 80% giá hiện tại hoặc giá mua (lấy mức thấp hơn). Sau 30 ngày: theo giá thỏa thuận.",
          },
          {
            productGroup: "Samsung Watch",
            exchangeTime: "30 ngày",
            returnPolicy:
              "Trong 30 ngày: hoàn tiền 80% giá hiện tại hoặc giá mua (lấy mức thấp hơn). Sau 30 ngày: theo giá thỏa thuận.",
          },
          {
            productGroup: "Laptop",
            exchangeTime: "30 ngày",
            returnPolicy:
              "Trong 30 ngày: hoàn tiền 80% giá hiện tại. Sau 30 ngày: không nhập lại.",
          },
          {
            productGroup: "Phụ kiện dưới 1 triệu",
            exchangeTime: "1 năm (mới) / 1 tháng (đã sử dụng)",
            returnPolicy: "Không nhập lại / hoàn tiền.",
          },
          {
            productGroup: "Phụ kiện từ 1 triệu trở lên",
            exchangeTime: "15 ngày",
            returnPolicy:
              "Trong 30 ngày: hoàn tiền 80% giá. Sau 30 ngày: theo giá thỏa thuận.",
          },
          {
            productGroup: "Gói bảo hành mở rộng",
            exchangeTime: "Không đổi",
            returnPolicy: "Trong 7 ngày (không lỗi): nhập lại 50% giá gói.",
          },
        ],
        conditionsTitle: "2. Điều kiện đổi mới / Hoàn tiền",
        conditions: [
          {
            text: "Máy mới: Như mới, không trầy xước, không dán decal, hình trang trí.",
          },
          {
            text: "Hộp: Như mới, không móp méo, rách, viết vẽ. Số Serial/IMEI trên hộp trùng với thân máy.",
          },
          {
            text: "Phụ kiện và quà tặng: Còn đầy đủ, nguyên vẹn, không móp méo hoặc hư hại.",
          },
          {
            text: "Tài khoản: Máy đã đăng xuất khỏi tất cả tài khoản (iCloud, Google, Mi Account...).",
          },
          {
            text: "Lỗi màn hình: Từ 3 điểm chết trở lên, hoặc 1 điểm chết kích thước lớn hơn 1mm (điện thoại) và từ 5 điểm chết trở lên (laptop, màn hình rời).",
          },
        ],
        standardTitle: "3. Bảo hành tiêu chuẩn",
        standardTableHeaders: {
          product: "Sản phẩm",
          warrantyTime: "Thời gian bảo hành",
          warrantyAddress: "Nơi bảo hành",
        },
        standardRows: [
          {
            product: "Hàng mới",
            warrantyTime: "12 tháng theo chính sách hãng",
            warrantyAddress: "TTBH chính hãng",
          },
          {
            product: "Hàng đã kích hoạt bảo hành chính hãng",
            warrantyTime: "12 tháng (BH còn lại tại hãng + BH tại IrusGear)",
            warrantyAddress: "TTBH chính hãng & IrusGear / Điện Thoại Vui",
          },
          {
            product: "Hàng đã sử dụng",
            warrantyTime: "6 tháng",
            warrantyAddress: "IrusGear / Điện Thoại Vui",
          },
        ],
        standardNote:
          "Phụ kiện đi kèm Vsmart, Asus, Nokia có thời gian bảo hành 6 tháng. Khách hàng có thể được hỗ trợ một điện thoại khác để sử dụng tạm trong thời gian chờ bảo hành.",
      },

      // IV - Cancellation
      cancellation: {
        title: "IV. Chính sách hủy giao dịch, đổi trả hàng",
        cancelOrderTitle: "1. Chính sách hủy đơn hàng",
        cancelOrderDesc:
          'Khách hàng có thể hủy đơn từ lúc "Đặt hàng" đến trước khi nhận hàng thành công.',
        cancelWays: [
          {
            text: "Gọi tổng đài: 1800.2097 (miền Nam) hoặc 1800.2044 (miền Bắc).",
          },
          { text: "Gửi email: support\\u0040irusgear.com." },
          { text: "Từ chối nhận hàng và xác nhận hủy với đơn vị giao hàng." },
        ],
        returnPolicyTitle: "2. Chính sách đổi trả",
        returnTableHeaders: {
          productGroup: "Nhóm sản phẩm",
          returnTime: "Thời gian đổi trả (Mới / Đã sử dụng)",
          returnFee: "Phí hoàn (nếu có)",
        },
        returnRows: [
          {
            productGroup: "Điện thoại, Máy tính bảng, Macbook",
            returnTime: "30 ngày / 30 ngày",
            returnFee: "20% giá trị (mới) / 15% giá trị (đã sử dụng)",
          },
          {
            productGroup: "Apple Watch",
            returnTime: "30 ngày / 30 ngày",
            returnFee: "20% / 20%",
          },
          {
            productGroup: "Samsung Watch",
            returnTime: "30 ngày / 30 ngày",
            returnFee: "30% / 30%",
          },
          {
            productGroup: "Laptop",
            returnTime: "30 ngày / 30 ngày",
            returnFee: "20% (mới) / Không áp dụng",
          },
          {
            productGroup: "Phụ kiện dưới 1 triệu",
            returnTime: "1 năm / 30 ngày",
            returnFee: "Không áp dụng",
          },
          {
            productGroup: "Phụ kiện từ 1 triệu trở lên",
            returnTime: "15 ngày / 15 ngày",
            returnFee: "Không áp dụng",
          },
          {
            productGroup: "Gói bảo hành mở rộng",
            returnTime: "Không đổi / Không trả",
            returnFee: "Phí 50% (trả trong 7 ngày)",
          },
        ],
        returnNote1: "* Riêng AirPods: phí hoàn 20%.",
        returnNote2:
          "* Đơn hàng online: thời gian tính từ ngày nhận hàng, không quá 7 ngày kể từ ngày xuất hóa đơn.",
        conditionsTitle: "3. Điều kiện đổi trả",
        conditions: [
          {
            text: "Máy: Như mới, không trầy xước, không móp méo, không bong tróc, không dính nước.",
          },
          {
            text: "Hộp: Nguyên vẹn, không rách, không ố vàng, Serial/IMEI trùng khớp.",
          },
          { text: "Phụ kiện & quà tặng: Còn đầy đủ, nguyên vẹn." },
          {
            text: "Tài khoản: Đã đăng xuất khỏi iCloud, Google, Mi Account...",
          },
        ],
        refundTitle: "4. Chính sách hoàn tiền",
        refundTableHeaders: {
          paymentMethod: "Phương thức thanh toán",
          processingTime: "Thời gian xử lý hoàn tiền",
        },
        refundRows: [
          {
            paymentMethod: "Tiền mặt",
            processingTime: "Hoàn ngay tại cửa hàng",
          },
          {
            paymentMethod: "Chuyển khoản ngân hàng",
            processingTime: "2 ngày làm việc",
          },
          { paymentMethod: "Thẻ ATM", processingTime: "7 – 10 ngày làm việc" },
          {
            paymentMethod: "Visa / Master / JCB",
            processingTime: "7 – 15 ngày làm việc",
          },
          {
            paymentMethod: "MPOS / ALEPAY",
            processingTime: "7 – 10 ngày làm việc",
          },
          { paymentMethod: "VNPAY", processingTime: "3 – 8 ngày làm việc" },
          { paymentMethod: "MOCA", processingTime: "3 – 5 ngày làm việc" },
        ],
        refundNote:
          "* Ngày làm việc: thứ 2 đến thứ 6 hàng tuần (không tính T7, CN, ngày lễ).",
      },

      // V - Privacy IrusGear
      privacyIrusgear: {
        title: "V. Chính sách bảo mật thông tin khách hàng IrusGear",
        collectPurposeTitle: "1. Mục đích thu thập thông tin",
        collectPurpose: [
          { text: "Xử lý đơn hàng và cung cấp dịch vụ cho khách hàng." },
          {
            text: "Gửi thông báo về tình trạng đơn hàng, khuyến mãi và sản phẩm mới (khi có sự đồng ý).",
          },
          {
            text: "Hỗ trợ khách hàng, giải quyết khiếu nại và các vấn đề liên quan.",
          },
          {
            text: "Giám sát an ninh, phòng chống gian lận và hoạt động bất hợp pháp.",
          },
          {
            text: "Thực hiện các nghĩa vụ pháp lý theo yêu cầu của cơ quan nhà nước có thẩm quyền.",
          },
        ],
        scopeTitle: "2. Phạm vi sử dụng thông tin",
        scope: [
          { text: "Thông tin được sử dụng nội bộ bởi IrusGear." },
          {
            text: "Không cung cấp thông tin cá nhân cho bên thứ ba khi chưa có sự đồng ý của khách hàng, trừ các trường hợp pháp luật yêu cầu.",
          },
        ],
        storageTitle: "3. Thời gian lưu trữ",
        storageDesc:
          "Thông tin được lưu trữ cho đến khi khách hàng yêu cầu xóa, tài khoản đóng hoặc theo quy định pháp luật.",
        rightsTitle: "4. Quyền của khách hàng",
        rights: [
          {
            text: "Truy cập, chỉnh sửa hoặc yêu cầu xóa thông tin cá nhân của mình.",
          },
          { text: "Rút lại sự đồng ý thu thập dữ liệu bất kỳ lúc nào." },
          { text: "Khiếu nại về việc xử lý thông tin cá nhân." },
        ],
        contactTitle: "5. Thông tin liên hệ",
        contact: [
          { text: "Công ty: IrusGear." },
          {
            text: "Địa chỉ: 350-352 Võ Văn Kiệt, Phường Cô Giang, Quận 1, TP. Hồ Chí Minh.",
          },
          { text: "Điện thoại: 1800.2097." },
          { text: "Email: support\\u0040irusgear.com." },
        ],
      },

      // VI - Privacy IrusGear Community
      privacySforum: {
        title: "VI. Chính sách bảo mật thông tin khách hàng IrusGear Community",
        infoTypesTitle: "1. Loại thông tin thu thập",
        infoTypes: [
          {
            text: "Thông tin cá nhân: Họ tên, số điện thoại, email, địa chỉ giao hàng, số CCCD (đối với một số giao dịch).",
          },
          {
            text: "Thông tin kỹ thuật: Địa chỉ IP, loại trình duyệt, thời gian truy cập, cookie để theo dõi sở thích và cải thiện hiệu suất Website.",
          },
          {
            text: "Thông tin giao dịch: Thông tin liên quan đến việc sử dụng dịch vụ và lịch sử trên Cộng đồng IrusGear.",
          },
        ],
        usagePurposeTitle: "2. Mục đích sử dụng dữ liệu",
        usagePurpose: [
          {
            text: "Quản lý tài khoản người dùng và cung cấp dịch vụ trên Cộng đồng IrusGear.",
          },
          {
            text: "Gửi thông báo về hoạt động tài khoản, khuyến mãi và sản phẩm mới (khi có sự đồng ý).",
          },
          { text: "Hỗ trợ khách hàng, xử lý sự cố và giải quyết khiếu nại." },
          {
            text: "Giám sát an ninh, phòng chống gian lận và hoạt động bất hợp pháp.",
          },
          {
            text: "Tuân thủ các yêu cầu pháp lý từ cơ quan nhà nước có thẩm quyền.",
          },
        ],
        storageTitle: "3. Thời gian lưu trữ",
        storageDesc:
          "Thông tin được lưu trữ cho đến khi người dùng yêu cầu xóa, tài khoản đóng hoặc theo quy định pháp luật.",
        managerTitle: "4. Đơn vị quản lý dữ liệu",
        manager: [
          { text: "Công ty: IrusGear." },
          {
            text: "Địa chỉ: 350-352 Võ Văn Kiệt, Phường Cô Giang, Quận 1, TP. Hồ Chí Minh.",
          },
          { text: "Điện thoại: 1800.2063." },
        ],
        rightsObligationsTitle: "5. Quyền và nghĩa vụ của người dùng",
        rightsObligations: [
          {
            text: "Quyền: Truy cập, chỉnh sửa, xóa hoặc hạn chế xử lý dữ liệu cá nhân. Rút lại sự đồng ý thu thập dữ liệu bất kỳ lúc nào.",
          },
          {
            text: "Nghĩa vụ: Cung cấp thông tin chính xác và chịu trách nhiệm bảo mật thông tin tài khoản (mật khẩu...).",
          },
        ],
        securityTitle: "6. Biện pháp bảo mật",
        security: [
          {
            text: "IrusGear sử dụng mã hóa (SSL/TLS) và máy chủ bảo mật để bảo vệ dữ liệu người dùng.",
          },
          {
            text: "Việc truy cập dữ liệu cá nhân được giới hạn đối với nhân viên được ủy quyền.",
          },
        ],
        complaintTitle: "7. Khiếu nại và thông tin liên hệ",
        complaint: [
          {
            text: "Người dùng có thể liên hệ bộ phận hỗ trợ để được giải đáp hoặc thực hiện yêu cầu liên quan đến dữ liệu cá nhân.",
          },
          { text: "Email: support\\u0040irusgear.com" },
          { text: "Điện thoại: 0824111196" },
          {
            text: "Chính sách có hiệu lực từ ngày 01/01/2024. IrusGear có quyền cập nhật chính sách và sẽ thông báo trên Website.",
          },
        ],
      },

      // VII - IrusGear Community agreement
      sforum: {
        title:
          "VII. Thỏa thuận cung cấp và sử dụng dịch vụ trên Cộng đồng IrusGear",
        article1Title: "Điều 1. Định nghĩa",
        article1: [
          {
            text: "Nhà cung cấp: IrusGear, hoạt động hợp pháp theo quy định pháp luật Việt Nam.",
          },
          {
            text: "Cộng đồng IrusGear: Phần của hệ thống trang thông tin điện tử, cung cấp dịch vụ mạng xã hội cho cộng đồng trao đổi, chia sẻ thông tin về công nghệ dưới các hình thức bình luận, chia sẻ bài viết.",
          },
          {
            text: "Người sử dụng: Cá nhân, tổ chức có tài khoản hợp lệ trên Cộng đồng IrusGear.",
          },
          {
            text: "Ban Quản trị: Bộ phận thuộc Nhà cung cấp, chịu trách nhiệm quản lý hoạt động Cộng đồng IrusGear và giải quyết vi phạm, khiếu nại.",
          },
        ],
        article2Title: "Điều 2. Đăng ký và ngừng sử dụng dịch vụ",
        article2: [
          {
            text: "Trước khi đăng ký, Người sử dụng xác nhận đã đọc, hiểu và đồng ý với Thỏa thuận này.",
          },
          {
            text: "Người sử dụng cam kết cung cấp thông tin chính xác, đầy đủ và trung thực. Vi phạm có thể dẫn đến tạm dừng hoặc chấm dứt cung cấp dịch vụ.",
          },
          {
            text: "Người sử dụng tự chịu trách nhiệm bảo mật mật khẩu. Nhà cung cấp không chịu trách nhiệm về thiệt hại phát sinh từ việc không bảo mật mật khẩu.",
          },
          {
            text: "Ban quản trị có quyền xóa tài khoản không sử dụng trong thời gian dài hoặc vi phạm nghiêm trọng Thỏa thuận.",
          },
        ],
        article3Title: "Điều 3. Nội dung cấm trao đổi, chia sẻ",
        article3: [
          {
            text: "Chống lại Nhà nước CHXHCN Việt Nam, gây phương hại an ninh quốc gia, trật tự an toàn xã hội.",
          },
          {
            text: "Kích động bạo lực, dâm ô, đồi trụy, tội ác, tệ nạn xã hội, mê tín dị đoan.",
          },
          {
            text: "Tiết lộ bí mật nhà nước, bí mật quân sự, an ninh, kinh tế, đối ngoại.",
          },
          {
            text: "Xuyên tạc, vu khống, xúc phạm uy tín tổ chức, danh dự và nhân phẩm cá nhân.",
          },
          {
            text: "Quảng cáo, tuyên truyền mua bán hàng hóa/dịch vụ bị cấm; truyền bá tác phẩm bị cấm.",
          },
          {
            text: "Đưa thông tin giả mạo, thông tin sai sự thật gây hoang mang dư luận.",
          },
          {
            text: "Vi phạm bản quyền sản phẩm, âm nhạc, video, phim ảnh, phần mềm.",
          },
          {
            text: "Nội dung kỳ thị dân tộc, giới tính, tôn giáo, chính trị, chia rẽ vùng miền.",
          },
          { text: "Các nội dung khác vi phạm pháp luật Việt Nam." },
        ],
        article4Title: "Điều 4. Quyền và nghĩa vụ của Người sử dụng",
        article4: [
          {
            text: "Quyền: Sử dụng dịch vụ, tính năng của Cộng đồng IrusGear; được bảo mật thông tin cá nhân; góp ý, phản ánh, khiếu nại.",
          },
          {
            text: "Nghĩa vụ: Tuân thủ Thỏa thuận và pháp luật; cung cấp thông tin chính xác; tự bảo mật tài khoản; chịu trách nhiệm nội dung bài viết, bình luận; không phá hoại hệ thống; không đăng tải nội dung cấm.",
          },
        ],
        article5Title: "Điều 5. Quyền của Nhà cung cấp",
        article5: [
          {
            text: "Thiết lập, thay đổi, nâng cấp, bảo trì hệ thống Cộng đồng IrusGear bất kỳ lúc nào mà không cần thông báo trước.",
          },
          {
            text: "Tạm ngừng hoặc chấm dứt cung cấp dịch vụ trong trường hợp bảo trì, sự cố kỹ thuật hoặc theo yêu cầu cơ quan nhà nước.",
          },
          {
            text: "Kiểm duyệt, chỉnh sửa hoặc xóa nội dung vi phạm; sử dụng nội dung người dùng cho mục đích truyền thông, quảng bá.",
          },
          {
            text: "Từ chối cung cấp dịch vụ hoặc xóa tài khoản vi phạm Thỏa thuận.",
          },
        ],
        article6Title: "Điều 6. Xử lý vi phạm",
        article6: [
          {
            text: "Xử lý dựa trên Thỏa thuận và pháp luật, đảm bảo công bằng, minh bạch.",
          },
          {
            text: "Các biện pháp xử lý: cảnh báo, khóa tài khoản tạm thời hoặc vĩnh viễn, xóa nội dung vi phạm tùy mức độ.",
          },
          {
            text: "Thông báo cho người dùng về vi phạm và hình thức xử lý qua email hoặc thông báo trên hệ thống.",
          },
        ],
        article7Title: "Điều 7. Cảnh báo rủi ro",
        article7:
          "Người sử dụng cần ý thức rằng việc chia sẻ thông tin trên môi trường mạng luôn tiềm ẩn rủi ro về bảo mật. Nhà cung cấp không chịu trách nhiệm về rủi ro phát sinh từ việc người dùng tự ý chia sẻ thông tin cá nhân. Người dùng nên cân nhắc kỹ trước khi đăng tải thông tin quan trọng hoặc mang tính riêng tư.",
        article8Title: "Điều 8. Giải quyết khiếu nại, tranh chấp",
        article8: [
          {
            text: "Mọi khiếu nại được giải quyết trên tinh thần hợp tác và thỏa thuận giữa các bên.",
          },
          {
            text: "Trường hợp không thỏa thuận được, các bên có quyền đưa ra cơ quan nhà nước có thẩm quyền.",
          },
          { text: "Hotline: 1800.2097, Email: support\\u0040irusgear.com" },
        ],
        article9Title: "Điều 9. Miễn trừ trách nhiệm pháp lý",
        article9: [
          {
            text: "Nhà cung cấp không chịu trách nhiệm về bất kỳ thiệt hại trực tiếp hoặc gián tiếp nào phát sinh từ việc sử dụng dịch vụ.",
          },
          {
            text: "Nhà cung cấp không chịu trách nhiệm về tính chính xác, tính đầy đủ của thông tin do người dùng đăng tải.",
          },
        ],
        article10Title: "Điều 10. Sửa đổi Thỏa thuận",
        article10:
          "Nhà cung cấp có quyền sửa đổi, bổ sung Thỏa thuận này bất kỳ lúc nào. Các thay đổi có hiệu lực kể từ thời điểm được đăng tải trên Website. Người dùng tiếp tục sử dụng dịch vụ sau khi có thay đổi đồng nghĩa với việc chấp nhận các điều khoản mới.",
        hotline: {
          purchaseLabel: "Gọi mua hàng",
          purchaseValue: "1800.2097 (8h00 – 22h00)",
          complaintLabel: "Gọi khiếu nại",
          complaintValue: "1800.2063 (8h00 – 21h30)",
          warrantyLabel: "Gọi bảo hành",
          warrantyValue: "1800.2064 (8h00 – 21h00)",
        },
      },
    },

    // === Shared / page content ===
    content: {
      sidebarTitle: "Hỗ trợ",
      loading: "Đang tải nội dung...",
      noContent: "Không có nội dung.",
      backToTop: " Quay lại đầu trang",
    },
  },
  admin: {
    layout: {
      brandName: "IrusGear",
      search: "Tìm kiếm",
      searchPlaceholder: "Tìm kiếm",
      searchShortcut: "Ctrl K",
      collapseMenu: "Thu gọn thanh bên",
      expandMenu: "Mở rộng thanh bên",
      openMenu: "Mở menu",
      closeMenu: "Đóng menu",
      logout: "Đăng xuất",
      settings: "Cài đặt",
      notifications: "Thông báo",
      apps: "Ứng dụng",
      language: "Ngôn ngữ",
      profile: "Hồ sơ",
    },
    notifications: {
      title: "Thông báo",
      markAllRead: "Đánh dấu đã đọc",
      loading: "Đang tải...",
      loadError: "Không thể tải thông báo",
      empty: "Không có thông báo",
      removeAria: "Xoá thông báo",
    },
    profile: {
      defaultName: "Quản trị viên",
      defaultRole: "Admin",
      profile: "Hồ sơ",
      settings: "Cài đặt",
      billingPlan: "Gói dịch vụ",
      pricing: "Bảng giá",
      faq: "Câu hỏi thường gặp",
      loggingOut: "Đang đăng xuất...",
      logout: "Đăng xuất",
    },
    bottomNav: {
      dashboard: "Tổng quan",
      products: "Sản phẩm",
      orders: "Đơn hàng",
      customers: "Khách hàng",
      analytics: "Phân tích",
      settings: "Cài đặt",
      more: "Thêm",
    },
    sidebar: {
      appsPages: "Ứng dụng & Trang",
      operations: "Vận hành",
      ecommerce: "Thương mại điện tử",
      dashboard: "Tổng quan",
      product: "Sản phẩm",
      productList: "Danh sách",
      productAdd: "Thêm mới",
      productCategory: "Danh mục",
      order: "Đơn hàng",
      orderList: "Danh sách",
      orderDetails: "Chi tiết",
      customer: "Khách hàng",
      customerList: "Danh sách",
      customerDetails: "Chi tiết",
      manageReview: "Quản lý đánh giá",
      referrals: "Giới thiệu",
      analytics: "Phân tích",
      inventory: "Kho hàng",
    },
    dashboard: {
      title: "Tổng quan",
      salesChannels: "Kênh bán hàng",
      revenueReport: "Báo cáo doanh thu",
      earningReports: "Báo cáo thu nhập",
      profit: "Lợi nhuận",
      expenseBreakdown: "Chi phí",
      leads: "Khách tiềm năng",
      budget: "Ngân sách",
      transactions: "Giao dịch",
      totalSales: "Tổng doanh số",
      heroTitle: "Chúc mừng",
      heroName: "Admin!",
      heroSubtitle: "Người bán hàng xuất sắc nhất tháng",
      viewSales: "Xem doanh số",
      statistics: "Thống kê",
      updatedAgo: "Cập nhật 1 tháng trước",
      expenses: "Chi phí",
      earning: "Thu nhập",
      expense: "Chi phí",
      budgetYear: "Năm ngân sách",
      budgetTarget: "Ngân sách: {amount}",
      increaseBudget: "Tăng ngân sách",
      generatedLeads: "Khách tiềm năng",
      monthlyReport: "Báo cáo tháng",
      total: "Tổng",
      weeklyOverview: "Tổng quan tuần",
      popularProducts: "Sản phẩm phổ biến",
      totalVisitors: "Tổng 10.4k lượt xem",
      salesCount: "{count} lượt bán",
      transactionsSubtitle: "Tổng 58 giao dịch trong tháng này",
      txTypes: {
        refund: "Hoàn tiền",
        vnpayPayment: "Thanh toán VNPay",
        momoPayment: "Thanh toán MoMo",
        codPayment: "Thanh toán COD",
        payment: "Thanh toán",
        deposit: "Nạp tiền",
        withdraw: "Rút tiền",
      },
      txDescriptions: {
        refundCustomer: "Khách trả hàng",
        orderRef: "Đơn hàng #{id}",
      },
      recentOrders: "Đơn hàng gần đây",
      recentOrdersSubtitle: "Các đơn mua mới nhất",
      viewAll: "Xem tất cả",
      moreEarningActions: "Thêm thao tác báo cáo thu nhập",
      moreProductActions: "Thêm thao tác sản phẩm phổ biến",
      moreTransactionActions: "Thêm thao tác giao dịch",
      moreTxActions: "Thêm thao tác",
      ordersByCountries: "Đơn hàng theo quốc gia",
      ordersByStatus: "Đơn hàng theo trạng thái",
      ordersByLocation: "Đơn hàng theo khu vực",
      locationSubtitle: "Doanh thu từ các khu vực hàng đầu",
      orders: "Đơn hàng",
      moreActions: "Thêm thao tác",
      countriesSubtitle: "62 đơn đang giao",
      moreCountryActions: "Thêm thao tác đơn hàng theo quốc gia",
      sender: "Người gửi",
      receiver: "Người nhận",
      footerMadeBy: "© 2026 Thiết kế bởi IrusGear",
      footerNavigation: "Điều hướng cuối dashboard",
      license: "Bản quyền",
      moreThemes: "Giao diện khác",
      documentation: "Tài liệu",
      support: "Hỗ trợ",
      quickSettings: "Cài đặt nhanh",
      buyNow: "Mua ngay",
      backToTop: "Lên đầu trang",
      stats: {
        sales: "Doanh số",
        customers: "Khách hàng",
        products: "Sản phẩm",
        revenue: "Doanh thu",
      },
      table: {
        order: "Đơn hàng",
        customer: "Khách hàng",
        date: "Ngày",
        status: "Trạng thái",
        amount: "Số tiền",
      },
      status: {
        pending: "Chờ xử lý",
        paid: "Đã thanh toán",
        failed: "Thất bại",
        cancelled: "Đã hủy",
        refunded: "Đã hoàn tiền",
      },
      leadSources: {
        organic: "Tự nhiên",
        ads: "Quảng cáo",
        referral: "Giới thiệu",
      },
      countryTabs: {
        new: "Mới",
        preparing: "Chuẩn bị",
        shipping: "Đang giao",
      },
      ordersByStatusSubtitle: "Tổng hợp trạng thái đơn hàng",
      totalOrders: "Tổng đơn hàng",
      salesChannelsSubtitle: "Doanh thu theo kênh",
      ordersCount: "{count} đơn",
    },
    products: {
      title: "Sản phẩm",
      addProduct: "Thêm sản phẩm",
      editProduct: "Sửa sản phẩm",
      productName: "Tên sản phẩm",
      product: "Sản phẩm",
      category: "Danh mục",
      stock: "Tồn kho",
      sku: "SKU",
      price: "Giá",
      qty: "SL",
      status: "Trạng thái",
      actions: "Thao tác",
      searchProduct: "Tìm sản phẩm",
      exportBtn: "Xuất file",
      filterStatusLabel: "Trạng thái",
      filterCategoryLabel: "Danh mục",
      filterStockLabel: "Tồn kho",
      allStatus: "Tất cả trạng thái",
      allCategories: "Tất cả danh mục",
      allStock: "Tất cả tồn kho",
      published: "Đã đăng",
      scheduled: "Đã lên lịch",
      inactive: "Không hoạt động",
      draft: "Bản nháp",
      inStock: "Còn hàng",
      lowStock: "Sắp hết",
      outOfStock: "Hết hàng",
      view: "Xem",
      edit: "Sửa",
      delete: "Xóa",
      duplicate: "Nhân bản",
      loadMore: "Tải thêm",
      ordersLabel: "đơn hàng",
      totalProducts: "Tổng sản phẩm",
      active: "Đang bán",
      productImageAlt: "Hình ảnh sản phẩm",
      addProductSubtitle: "Tạo sản phẩm mới cho cửa hàng của bạn",
      editProductSubtitle: "Chỉnh sửa thông tin sản phẩm",
      productNotFound: "Không tìm thấy sản phẩm",
      productNotFoundDesc: "Sản phẩm bạn đang tìm không tồn tại.",
      backToProducts: "Quay lại danh sách sản phẩm",
      discard: "Hủy bỏ",
      saveDraft: "Lưu nháp",
      publishProduct: "Đăng sản phẩm",
      saveChanges: "Lưu thay đổi",
      basicInformation: "Thông tin cơ bản",
      productNamePlaceholder: "Ví dụ: iPhone 15 Pro Max",
      slug: "Đường dẫn",
      slugPlaceholder: "iphone-15-pro-max",
      skuPlaceholder: "IRG-00001",
      vendor: "Nhà cung cấp",
      vendorPlaceholder: "IrusGear",
      description: "Mô tả",
      productDescription: "Mô tả sản phẩm",
      descriptionPlaceholder: "Viết mô tả chi tiết...",
      media: "Hình ảnh",
      dragAndDrop: "Kéo thả hoặc",
      browseUpload: "chọn tệp",
      toUpload: "để tải lên",
      uploadHint: "PNG, JPG, WEBP tối đa 5MB",
      primary: "Chính",
      variants: "Biến thể",
      addVariant: "Thêm biến thể",
      noVariants: "Chưa có biến thể nào",
      variantsHint: "Thêm biến thể như kích cỡ, màu sắc hoặc chất liệu",
      pricing: "Giá bán",
      compareAtPrice: "Giá so sánh",
      inventory: "Kho hàng",
      quantity: "Số lượng",
      trackInventory: "Theo dõi kho",
      trackInventoryDesc: "Bật quản lý tồn kho cho sản phẩm này",
      productCategory: "Danh mục sản phẩm",
      selectCategory: "Chọn danh mục",
      featuredProduct: "Sản phẩm nổi bật",
      featuredProductDesc: "Hiển thị trong các khu vực nổi bật",
      productStatus: "Trạng thái sản phẩm",
      seoPreview: "Xem trước SEO",
      seoProductName: "Tên sản phẩm",
      seoProductSlug: "duong-dan-san-pham",
      seoDescPlaceholder: "Mô tả sản phẩm sẽ hiển thị ở đây...",
      nameRequired: "Vui lòng nhập tên sản phẩm",
      skuRequired: "Vui lòng nhập SKU",
      priceRequired: "Giá phải lớn hơn 0",
      deleteConfirm: 'Xóa sản phẩm "{name}"?',
      deleteConfirmEdit: 'Xóa "{name}"? Hành động này không thể hoàn tác.',
      deleteFailed: "Xóa thất bại: {message}",
      saveFailed: "Lưu thất bại: {message}",
      publishFailed: "Đăng sản phẩm thất bại: {message}",
      saveDraftFailed: "Lưu bản nháp thất bại: {message}",
      duplicateAction: "Nhân bản sản phẩm #{id}",
      exportTriggered: "Đã kích hoạt xuất file",
      duplicatePrefilled: "Đã sao chép thông tin từ sản phẩm gốc — chỉnh sửa rồi lưu để tạo bản mới",
      noDataToExport: "Không có dữ liệu để xuất",
      exportSuccess: "Đã xuất {count} sản phẩm ra CSV",
      headTitle: "Sản phẩm – IrusGear Admin",
      headTitleAdd: "Thêm sản phẩm – IrusGear Admin",
      headTitleEdit: "Sửa {name} – IrusGear Admin",
      headTitleNotFound: "Không tìm thấy sản phẩm",
    },
    categories: {
      title: "Danh mục",
      addCategory: "Thêm danh mục",
      editCategory: "Sửa danh mục",
      category: "Danh mục",
      categoryName: "Tên danh mục",
      totalProducts: "Tổng sản phẩm",
      totalEarning: "Tổng thu nhập",
      description: "Mô tả",
      searchCategory: "Tìm danh mục",
      status: "Trạng thái",
      active: "Hoạt động",
      inactive: "Không hoạt động",
      viewProducts: "Xem sản phẩm",
      delete: "Xóa",
      name: "Tên",
      slug: "Đường dẫn",
      categoryNamePlaceholder: "Tên danh mục",
      slugPlaceholder: "duong-dan-danh-muc",
      descriptionPlaceholder: "Mô tả ngắn",
      iconLabel: "Biểu tượng (lớp BI)",
      iconPlaceholder: "bi-phone",
      cancel: "Hủy",
      createCategory: "Tạo danh mục",
      saveChanges: "Lưu thay đổi",
      deleteConfirm: 'Xóa danh mục "{name}"?',
      deleteFailed: "Xóa thất bại: {message}",
      saveFailed: "Lưu thất bại: {message}",
      viewProductsAlert: 'Xem sản phẩm trong "{name}"',
      headTitle: "Danh mục – IrusGear Admin",
      edit: "Sửa",
      pageTitle: "Danh mục – IrusGear Admin",
    },
    orders: {
      title: "Đơn hàng",
      orderId: "Mã đơn",
      order: "Đơn hàng",
      customer: "Khách hàng",
      date: "Ngày",
      payment: "Thanh toán",
      fulfillment: "Giao hàng",
      status: "Trạng thái",
      method: "Phương thức",
      total: "Tổng",
      actions: "Thao tác",
      viewDetails: "Xem chi tiết",
      delete: "Xoá",
      loadMore: "Tải thêm",
      searchOrder: "Tìm đơn hàng",
      orderDetails: "Chi tiết đơn hàng",
      shippingActivity: "Hoạt động giao hàng",
      customerDetails: "Thông tin khách hàng",
      shippingAddress: "Địa chỉ giao hàng",
      billingAddress: "Địa chỉ thanh toán",
      paymentMethod: "Phương thức thanh toán",
      cashOnDelivery: "Thanh toán khi nhận hàng",
      notAvailable: "N/A",
      guest: "Khách",
      pendingPayment: "Chờ thanh toán",
      completed: "Đã hoàn tất",
      refunded: "Đã hoàn tiền",
      failed: "Thất bại",
      ordersMeta: "đơn hàng",
      chips: {
        all: "Tất cả",
        confirmed: "Chờ duyệt",
        processing: "Đang xử lý",
        ready_to_ship: "Sẵn sàng giao",
        shipped: "Đã lấy hàng",
        delivering: "Đang giao",
        delivered: "Đã giao",
        delivery_failed: "Giao thất bại",
        returned: "Đã trả",
        cancelled: "Đã hủy",
      },
      approval: {
        title: "Duyệt đơn hàng",
        subtitle: "Kiểm tra nhanh trước khi chuyển đơn sang xử lý.",
        checkPayment: "Thanh toán hợp lệ",
        checkStock: "Hàng và tồn kho sẵn sàng",
        checkFraud: "Không có dấu hiệu rủi ro",
        approve: "Duyệt",
        reject: "Từ chối",
        reasonPlaceholder: "Nhập lý do từ chối...",
        reasonRequired: "Vui lòng nhập lý do từ chối",
        confirmReject: "Xác nhận từ chối",
        approved: "Đã duyệt đơn hàng",
        rejected: "Đã từ chối đơn hàng",
        actionFailed: "Thao tác thất bại: {msg}",
        refundTitle: "Xác nhận hoàn tiền",
        refundWarning: "Đơn này đã thanh toán. Khi từ chối, hệ thống sẽ khởi tạo quy trình hoàn tiền cho khách.",
      },
      assign: {
        title: "Phân công shipper",
        subtitle: "Chọn nhân viên giao hàng đang hoạt động cho shipment này.",
        searchLabel: "Tìm kiếm",
        searchPlaceholder: "Tên, số điện thoại hoặc email",
        shipperLabel: "Shipper",
        selectPlaceholder: "Chọn shipper",
        activeCount: "{count} shipment đang xử lý",
        noZones: "Chưa có khu vực lấy hàng",
        refresh: "Tải lại",
        assign: "Phân công",
        reassign: "Phân công lại",
        assigned: "Đã phân công shipment",
        reassigned: "Đã phân công lại shipment",
        failed: "Phân công thất bại: {msg}",
      },
      timeline: {
        empty: "Chưa có hoạt động giao hàng",
        photoAlt: "Ảnh xác nhận giao hàng",
        statusUpdated: "Cập nhật trạng thái",
        eventLabels: {
          pending: "Đơn hàng đã được tạo",
          awaiting_payment: "Chờ thanh toán",
          confirmed: "Đơn hàng đã được xác nhận",
          processing: "Đơn hàng đang được xử lý",
          ready_to_ship: "Đơn hàng sẵn sàng giao",
          shipped: "Đơn vị giao hàng đã lấy hàng",
          delivering: "Đơn hàng đang được giao",
          delivered: "Đã giao hàng thành công",
          delivery_failed: "Giao hàng chưa thành công",
          cancelled: "Đơn hàng đã bị hủy",
          refunding: "Đang hoàn tiền",
          refunded: "Đã hoàn tiền",
          returned: "Đơn hàng đã được trả lại",
        },
        attempt: "Lần giao hàng",
        attemptTitle: "Lần giao #{number}",
        recipient: "Người nhận",
        codCollected: "COD đã thu",
        location: "Tọa độ",
        actor: {
          admin: "Quản trị viên",
          delivery: "Nhân viên giao hàng",
          gateway: "Cổng thanh toán",
          system: "Hệ thống",
          user: "Khách hàng",
        },
        outcome: {
          delivered: "Giao thành công",
          failed_not_home: "Khách không có nhà",
          failed_refused: "Khách từ chối nhận",
          failed_wrong_address: "Sai địa chỉ",
          failed_damaged: "Hàng hỏng",
          failed_other: "Giao thất bại",
        },
      },
      exportMock: "Đã kích hoạt xuất file (mock)",
      deleteMock: "Xoá đơn hàng {code} (mock)",
      noDataToExport: "Không có dữ liệu để xuất",
      exportSuccess: "Đã xuất {count} đơn hàng ra CSV",
      pageTitle: "Đơn hàng – IrusGear Admin",
      orderPageTitle: "Đơn hàng {code} – IrusGear Admin",
      loadingOrder: "Đang tải đơn hàng...",
      statusReadyToPickup: "Sẵn sàng lấy hàng",
      statusOutForDelivery: "Đang giao",
      statusDispatched: "Đã gửi",
      statusProcessing: "Đang xử lý",
      statusDelivered: "Đã giao",
      product: "Sản phẩm",
      price: "Giá",
      qty: "SL",
      subtotal: "Tạm tính",
      discount: "Giảm giá",
      shipping: "Phí vận chuyển",
      free: "Miễn phí",
      tax: "Thuế (8%)",
      note: "Ghi chú",
      edit: "Sửa",
      cancelOrder: "Hủy đơn hàng",
      orderNotFound: "Không tìm thấy đơn hàng",
      orderNotFoundDesc: "Đơn hàng bạn đang tìm không tồn tại.",
      backToOrders: "Quay lại danh sách đơn hàng",
      customerIdLabel: "Mã khách hàng: {code}",
      confirmCancel: "Hủy đơn hàng {code}? Hành động này không thể hoàn tác.",
      cancelSuccess: "Đã hủy đơn hàng thành công",
      cancelFailed: "Hủy đơn hàng thất bại: {msg}",
      cancelReason: "Đã hủy bởi quản trị viên",
      exportTriggered: "Đã kích hoạt xuất file (mock)",
      confirmDeleteOrder: "Xoá đơn hàng {code} (mock)",
      orderPrefix: "Đơn hàng",
    },
    customers: {
      title: "Khách hàng",
      customerId: "Mã khách hàng",
      country: "Quốc gia",
      totalOrders: "Đơn hàng",
      totalSpent: "Tổng chi tiêu",
      searchCustomer: "Tìm khách hàng",
      addCustomer: "Thêm khách hàng",
      customerDetails: "Chi tiết khách hàng",
      memberSince: "Thành viên từ",
      loyaltyProgram: "Chương trình khách hàng thân thiết",
      accountBalance: "Số dư tài khoản",
      wishlist: "Yêu thích",
      coupons: "Mã giảm giá",
      ordersPlaced: "Đơn đã đặt",
      editDetails: "Sửa thông tin",
      deleteCustomer: "Xóa khách hàng",
      pageTitle: "Khách hàng – IrusGear Admin",
      customerPageTitle: "Khách hàng {code} – IrusGear Admin",
      loadingCustomer: "Đang tải khách hàng...",
      viewDetails: "Xem chi tiết",
      edit: "Sửa",
      delete: "Xoá",
      loadMore: "Tải thêm",
      orders: "Đơn hàng",
      spent: "Đã chi",
      idLabel: "Mã",
      exportMock: "Đã kích hoạt xuất file (mock)",
      addMock: "Thêm khách hàng (mock)",
      noDataToExport: "Không có dữ liệu để xuất",
      exportSuccess: "Đã xuất {count} khách hàng ra CSV",
      addSuccess: "Đã thêm khách hàng mới",
      addFailed: "Thêm khách hàng thất bại",
      passwordHint: "Tối thiểu 8 ký tự, có chữ hoa, chữ thường và số",
      confirmDelete: "Xoá khách hàng {code}? Hành động này không thể hoàn tác.",
      deleteSuccess: "Đã xoá khách hàng thành công",
      deleteShort: "Đã xoá khách hàng",
      deleteFailed: "Không thể xoá khách hàng: {msg}",
      deleteFailedShort: "Không thể xoá: {msg}",
      defaultCountry: "Việt Nam",
      customerNotFound: "Không tìm thấy khách hàng",
      backToCustomers: "Quay lại danh sách khách hàng",
      customerIdHeader: "Mã khách hàng {code}",
      details: "Chi tiết",
      username: "Tên đăng nhập:",
      email: "Email:",
      statusLabel: "Trạng thái:",
      contact: "Liên hệ:",
      countryLabel: "Quốc gia:",
      pointsEarned: "điểm tích lũy",
      highestTier: "Đã đạt hạng cao nhất!",
      ptsToNext: "{pts} điểm để lên {tier}",
      tabOverview: "Tổng quan",
      tabSecurity: "Bảo mật",
      tabAddress: "Địa chỉ & Thanh toán",
      tabNotifications: "Thông báo",
      pts: "điểm",
      items: "sản phẩm",
      available: "còn lại",
      noOrdersFound: "Không có đơn hàng nào",
      order: "Đơn hàng",
      date: "Ngày",
      status: "Trạng thái",
      total: "Tổng",
      changePassword: "Đổi mật khẩu",
      newPassword: "Mật khẩu mới",
      confirmPassword: "Xác nhận mật khẩu",
      updatePassword: "Cập nhật mật khẩu",
      twoFactor: "Xác thực hai yếu tố",
      enable2fa: "Bật 2FA",
      enable2faDesc: "Thêm lớp bảo mật cho tài khoản này",
      notifEmailOrder: "Email khi có đơn mới",
      notifEmailOrderDesc: "Gửi email khi khách hàng này đặt đơn",
      notifEmailPromo: "Email khuyến mãi",
      notifEmailPromoDesc: "Bao gồm trong chiến dịch tiếp thị",
      notifEmailAccount: "Cảnh báo đăng nhập (Telegram cho admin)",
      notifEmailAccountDesc: "Gửi log Telegram cho admin khi khách hàng này đăng nhập",
      notifSmsOrder: "SMS cập nhật đơn hàng",
      notifSmsOrderDesc: "Gửi SMS khi đơn hàng đổi trạng thái",
      notifMockBadge: "Mock",
      notifMockToast: "Tính năng này đang trong roadmap, chưa hoạt động.",
      notifUpdateSuccess: "Đã cập nhật cài đặt thông báo",
      notifUpdateFailed: "Không thể cập nhật cài đặt. Vui lòng thử lại.",
      editDetailsMock: "Sửa thông tin (mock)",
      addCustomerMock: "Thêm khách hàng (mock)",
      exportTriggered: "Đã kích hoạt xuất file (mock)",
      standardTier: "Tiêu chuẩn",
      goldTier: "Vàng",
      platinumTier: "Bạch kim",
      quickEditTitle: "Sửa nhanh khách hàng",
      quickEditFirstNamePh: "Nhập tên",
      quickEditLastNamePh: "Nhập họ",
      quickEditEmailPh: "Nhập email",
      quickEditPhonePh: "Nhập số điện thoại",
      quickEditEmailRequired: "Vui lòng nhập email.",
      quickEditEmailInvalid: "Email không hợp lệ.",
      quickEditPhoneInvalid: "Số điện thoại không hợp lệ.",
      quickEditStatusHint: "Inactive hoặc Chưa verify sẽ chặn khách hàng đăng nhập.",
      emailVerifyLabel: "Xác thực email",
      emailVerifyYes: "Đã verify",
      emailVerifyNo: "Chưa verify",
      quickEditResetPassword: "Đặt lại mật khẩu",
      quickEditPasswordPh: "Để trống nếu không đổi",
      quickEditPasswordHint: "Tối thiểu 8 ký tự, có chữ hoa, chữ thường và số",
      quickEditPasswordTooShort: "Mật khẩu cần tối thiểu 8 ký tự.",
      quickEditPasswordWeak: "Mật khẩu cần có chữ hoa, chữ thường và số.",
      quickEditAddress: "Địa chỉ mặc định",
      quickEditAddressEmpty: "Khách hàng chưa có địa chỉ.",
      quickEditAddressHint: "Để sửa địa chỉ chi tiết (tỉnh/quận/phường), vào tab Địa chỉ ở trang chi tiết khách hàng.",
      quickEditAddAddress: "Thêm địa chỉ",
      quickEditAddressCreated: "Đã thêm địa chỉ cho khách hàng",
      quickEditAddressCreateFailed: "Không thể thêm địa chỉ. Vui lòng thử lại.",
      roleLabel: "Vai trò",
      updateAvatar: "Cập nhật ảnh đại diện",
      avatarUpdateSuccess: "Cập nhật ảnh đại diện thành công",
      avatarUpdateFailed: "Cập nhật ảnh đại diện thất bại",
      quickEditSave: "Lưu thay đổi",
      quickEditSaving: "Đang lưu...",
      quickEditSuccess: "Đã cập nhật thông tin khách hàng",
      quickEditFailed: "Không thể cập nhật. Vui lòng thử lại.",
      statusActive: "Đang hoạt động",
      statusInactive: "Tạm ngưng",
      statusPending: "Chờ duyệt",
      statusBanned: "Đã khóa",
    },
    reviews: {
      title: "Quản lý đánh giá",
      totalReviews: "Tổng đánh giá",
      published: "Đã hiển thị",
      pending: "Chờ duyệt",
      rejected: "Đã từ chối",
      reported: "Bị báo cáo",
      avgRating: "Đánh giá trung bình",
      product: "Sản phẩm",
      customer: "Khách hàng",
      rating: "Điểm đánh giá",
      review: "Đánh giá",
      status: "Trạng thái",
      date: "Ngày",
      searchReviews: "Tìm đánh giá",
      allStatuses: "Tất cả trạng thái",
      allRatings: "Tất cả điểm",
      reportedOnly: "Chỉ review bị báo cáo",
      approve: "Duyệt",
      reject: "Từ chối",
      viewDetails: "Xem chi tiết",
      pageTitle: "Quản lý đánh giá – IrusGear Admin",
      loadMore: "Tải thêm",
      outOf5: "trên 5",
      allTime: "toàn thời gian",
      reviewsMeta: "đánh giá",
      ratingLabel: "Điểm",
      reviewLabel: "Đánh giá",
      approvedMock: "Đã duyệt đánh giá",
      rejectedMock: "Đã từ chối đánh giá",
      actionFailed: "Không thể thực hiện: {error}",
      actionMock: "{label} đánh giá #{id} (mock)",
      exportMock: "Xuất đánh giá (mock)",
      noDataToExport: "Không có dữ liệu để xuất",
      exportSuccess: "Đã xuất {count} đánh giá ra CSV",
      unknownProduct: "Sản phẩm không xác định",
      anonymous: "Ẩn danh",
      noContent: "Không có nội dung",
      reports: "báo cáo",
      verifiedPurchase: "Đã mua hàng",
      yes: "Có",
      no: "Không",
      detailTitle: "Chi tiết đánh giá",
      helpfulCount: "Lượt hữu ích",
      rejectReview: "Từ chối đánh giá",
      rejectReason: "Lý do từ chối",
      rejectReasonPlaceholder: "Nhập lý do để admin có thể theo dõi vì sao review bị ẩn",
      reportDetails: "Nội dung báo cáo",
      noReports: "Chưa có báo cáo nào",
      cancel: "Hủy",
      approveSuccess: "Đã hiển thị đánh giá",
      rejectSuccess: "Đã ẩn đánh giá",
    },
    referrals: {
      title: "Giới thiệu",
      totalReferrals: "Tổng lượt giới thiệu",
      converted: "Đã chuyển đổi",
      activeLinks: "Liên kết đang hoạt động",
      totalEarned: "Tổng thu nhập",
      referrer: "Người giới thiệu",
      referredUser: "Người được giới thiệu",
      code: "Mã",
      earning: "Thu nhập",
      status: "Trạng thái",
      date: "Ngày",
      searchReferrals: "Tìm lượt giới thiệu",
      active: "Hoạt động",
      expired: "Hết hạn",
      revoke: "Thu hồi",
      pageTitle: "Giới thiệu – IrusGear Admin",
      viewDetails: "Xem chi tiết",
      loadMore: "Tải thêm",
      allTime: "toàn thời gian",
      referralsMeta: "lượt giới thiệu",
      commissionMeta: "hoa hồng",
      codeLabel: "Mã",
      earningLabel: "Thu nhập",
      dateLabel: "Ngày",
      unknown: "Không xác định",
      confirmRevoke: "Bạn có chắc muốn thu hồi lượt giới thiệu này?",
      actionMock: "{label} lượt giới thiệu #{id}",
      exportReferrals: "Xuất lượt giới thiệu (mock)",
      noDataToExport: "Không có dữ liệu để xuất",
      exportSuccess: "Đã xuất {count} lượt giới thiệu ra CSV",
      statusLabel: "Trạng thái",
    },
    analytics: {
      title: "Phân tích",
      subtitle: "Tổng quan hiệu suất và số liệu cửa hàng",
      totalRevenue: "Tổng doanh thu",
      totalOrders: "Tổng đơn hàng",
      avgOrderValue: "Giá trị đơn hàng trung bình",
      conversionRate: "Tỷ lệ chuyển đổi",
      revenueTrend: "Xu hướng doanh thu",
      salesByCategory: "Doanh số theo danh mục",
      conversionFunnel: "Phễu chuyển đổi",
      returningCustomers: "Khách hàng quay lại",
      topProducts: "Sản phẩm nổi bật",
      period7d: "7 ngày qua",
      period30d: "30 ngày qua",
      period90d: "90 ngày qua",
      period1y: "1 năm qua",
      returning: "Quay lại",
      newCustomer: "Mới",
      product: "Sản phẩm",
      unitsSold: "Đã bán",
      revenue: "Doanh thu",
      conversion: "Tỷ lệ",
    },
    settings: {
      title: "Cài đặt",
      account: "Tài khoản",
      security: "Bảo mật",
      billingPlans: "Thanh toán & Gói dịch vụ",
      notifications: "Thông báo",
      connections: "Kết nối",
      saveChanges: "Lưu thay đổi",
      cancel: "Hủy",
      deleteAccount: "Xóa tài khoản",
      changePassword: "Đổi mật khẩu",
      twoFactor: "Xác thực hai yếu tố",
      activeSessions: "Phiên hoạt động",
      currentPlan: "Gói hiện tại của bạn",
      paymentMethods: "Phương thức thanh toán",
      connectedAccounts: "Tài khoản đã kết nối",
      uploadNewPhoto: "Tải ảnh mới",
      reset: "Đặt lại",
      photoAllowed: "Cho phép JPG, GIF hoặc PNG. Tối đa 800kB",
      firstName: "TÊN",
      lastName: "HỌ",
      email: "EMAIL",
      organization: "TỔ CHỨC",
      phoneNumber: "SỐ ĐIỆN THOẠI",
      address: "ĐỊA CHỈ",
      state: "TỈNH/THÀNH",
      zipCode: "MÃ BƯU ĐIỆN",
      country: "QUỐC GIA",
      language: "NGÔN NGỮ",
      timezone: "MÚI GIỜ",
      currency: "TIỀN TỆ",
      saving: "Đang lưu...",
      deleteAccountDesc: "Sau khi xóa tài khoản, bạn không thể khôi phục. Hãy cân nhắc kỹ.",
      confirmDeactivate: "Tôi xác nhận hủy kích hoạt tài khoản",
      deactivateAccount: "Hủy kích hoạt tài khoản",
      currentPassword: "MẬT KHẨU HIỆN TẠI",
      newPassword: "MẬT KHẨU MỚI",
      confirmNewPassword: "XÁC NHẬN MẬT KHẨU MỚI",
      passwordRequirements: "YÊU CẦU MẬT KHẨU:",
      passwordReq1: "Tối thiểu 8 ký tự – càng dài càng tốt",
      passwordReq2: "Ít nhất một chữ thường",
      passwordReq3: "Ít nhất một chữ số, ký hiệu hoặc khoảng trắng",
      twoFactorDesc: "Thêm lớp bảo mật cho tài khoản bằng xác thực hai yếu tố.",
      activeSessionsDesc: "Các thiết bị đang đăng nhập vào tài khoản của bạn.",
      current: "Hiện tại",
      revoke: "Thu hồi",
      authenticatorApp: "Ứng dụng xác thực",
      authenticatorAppDesc: "Sử dụng ứng dụng xác thực để tạo mã một lần.",
      smsVerification: "Xác thực SMS",
      smsVerificationDesc: "Dùng số điện thoại để nhận mã xác thực.",
      setUp: "Thiết lập",
      currentPlanBtn: "Gói hiện tại",
      upgrade: "Nâng cấp",
      planBasic: "Cơ bản",
      planStandard: "Tiêu chuẩn",
      planEnterprise: "Doanh nghiệp",
      planFree: "Miễn phí",
      planBasicDesc: "Cho cá nhân",
      planStandardDesc: "Cho nhóm nhỏ",
      planEnterpriseDesc: "Cho tổ chức lớn",
      edit: "Sửa",
      delete: "Xóa",
      addPaymentMethod: "Thêm phương thức thanh toán",
      notificationPreferences: "Tùy chọn thông báo",
      notificationType: "Loại",
      notificationEmail: "Email",
      notificationBrowser: "Trình duyệt",
      notificationApp: "Ứng dụng",
      notifNewOrders: "Đơn hàng mới",
      notifOrderStatus: "Cập nhật trạng thái đơn",
      notifSecurity: "Cảnh báo bảo mật",
      connectedAccountsDesc: "Quản lý các dịch vụ và ứng dụng bên thứ ba đã kết nối.",
      connGoogleDesc: "Tích hợp Calendar, Drive và Gmail",
      connStripeDesc: "Xử lý thanh toán",
      profileUpdated: "Cập nhật hồ sơ thành công",
      profileUpdateFailed: "Cập nhật hồ sơ thất bại",
      fillPasswordFields: "Vui lòng điền đầy đủ các trường mật khẩu",
      passwordsNoMatch: "Mật khẩu mới không khớp",
      passwordChanged: "Đổi mật khẩu thành công",
      passwordChangeFailed: "Đổi mật khẩu thất bại",
      countryVietnam: "Việt Nam",
      countryUnitedStates: "Hoa Kỳ",
      countryJapan: "Nhật Bản",
      langEnglish: "English",
      langVietnamese: "Tiếng Việt",
      langFrench: "Français",
      tzHoChiMinh: "Asia/Ho Chi Minh (UTC +7)",
      tzNewYork: "America/New_York (UTC -5)",
      currencyVnd: "VND – Đồng Việt Nam",
      currencyUsd: "USD – Đô la Mỹ",
      pageTitle: "Cài đặt – IrusGear Admin",
      deviceChromeWindows: "Chrome trên Windows",
      deviceSafariIphone: "Safari trên iPhone",
      sessionLocation: "TP. Hồ Chí Minh, VN",
      sessionActiveNow: "Đang hoạt động",
      sessionTwoHoursAgo: "2 giờ trước",
      visaCard: "Visa kết thúc bằng 4242",
      visaCardExpires: "Hết hạn 12/2027",
      paypal: "PayPal",
      paypalEmail: "null",
      connGoogle: "Google",
      connStripe: "Stripe",
      defaultOrg: "IrusGear Corp.",
      defaultAddress: "45 Đường Nguyễn Huệ",
      defaultState: "TP. Hồ Chí Minh",
      defaultZip: "70000",
    },
    status: {
      active: "Hoạt động",
      inactive: "Không hoạt động",
      blocked: "Bị khóa",
      banned: "Bị khóa",
      pending: "Đang chờ",
      awaiting_payment: "Chờ thanh toán",
      paid: "Đã thanh toán",
      confirmed: "Đã xác nhận",
      ready_to_ship: "Sẵn sàng giao",
      ready_to_pickup: "Sẵn sàng lấy hàng",
      out_for_delivery: "Đang giao",
      dispatched: "Đã gửi",
      published: "Đã hiển thị",
      approved: "Đã hiển thị",
      rejected: "Đã từ chối",
      completed: "Hoàn thành",
      processing: "Đang xử lý",
      shipped: "Đã gửi hàng",
      delivering: "Đang giao hàng",
      delivered: "Đã giao hàng",
      delivery_failed: "Giao hàng thất bại",
      cancelled: "Đã hủy",
      refunding: "Đang hoàn tiền",
      refunded: "Đã hoàn tiền",
      returned: "Đã trả hàng",
      failed: "Thất bại",
      converted: "Đã chuyển đổi",
      expired: "Hết hạn",
      inStock: "Còn hàng",
      outOfStock: "Hết hàng",
      scheduled: "Đã lên lịch",
    },
    actions: {
      view: "Xem",
      edit: "Sửa",
      delete: "Xóa",
      exportBtn: "Xuất file",
      search: "Tìm kiếm",
      filter: "Lọc",
      save: "Lưu",
      cancel: "Hủy",
      add: "Thêm",
      remove: "Xóa",
      approve: "Duyệt",
      reject: "Từ chối",
      viewDetails: "Xem chi tiết",
    },
  },
  shipper: {
    layout: {
      brandName: "IrusGear",
      searchPlaceholder: "Tìm đơn hàng",
      collapseMenu: "Thu gọn menu",
      expandMenu: "Mở rộng menu",
      openMenu: "Mở menu",
      closeMenu: "Đóng menu",
      logout: "Đăng xuất",
      login: "Đăng nhập",
      notifications: "Thông báo",
    },
    sidebar: {
      workspace: "Khu làm việc",
      account: "Tài khoản",
      dashboard: "Tổng quan",
      pendingOrders: "Đơn chưa giao",
      deliveredOrders: "Đơn đã giao",
      earnings: "Thu nhập",
      profile: "Hồ sơ",
      settings: "Cài đặt",
    },
    bottomNav: {
      dashboard: "Tổng quan",
      pending: "Chưa giao",
      delivered: "Đã giao",
      earnings: "Thu nhập",
      profile: "Hồ sơ",
    },
    dashboard: {
      title: "Tổng quan",
      greeting: "Xin chào, {name}",
      greetingSub: "Chúc một ngày giao hàng suôn sẻ",
      statusOnline: "Đang nhận đơn",
      statusOffline: "Tạm nghỉ",
      viewAll: "Xem tất cả",
      empty: "Không có đơn cần giao",
      metrics: {
        totalEarnings: "Tổng tiền kiếm được",
        totalEarningsMeta: "Tổng cộng dồn",
        pendingOrders: "Đơn hàng chưa giao",
        pendingOrdersMeta: "Đang chờ giao",
        deliveredOrders: "Đơn hàng đã giao",
        deliveredOrdersMeta: "Hoàn thành tháng này",
        todayEarnings: "Thu nhập hôm nay",
        todayEarningsMeta: "Cập nhật theo phiên",
      },
      earningsChart: {
        title: "Thu nhập theo ngày",
        subtitle: "Theo dõi thu nhập của bạn",
        last7Days: "7 ngày qua",
        last30Days: "30 ngày qua",
      },
      statusBreakdown: {
        title: "Trạng thái đơn hàng",
        subtitle: "Phân bổ theo trạng thái",
        totalOrders: "Tổng đơn",
      },
      pendingList: {
        title: "Đơn hàng cần giao",
        subtitle: "Danh sách các đơn ưu tiên",
      },
      table: {
        order: "Mã đơn",
        customer: "Khách hàng",
        address: "Địa chỉ",
        status: "Trạng thái",
        fee: "Phí ship",
      },
    },
    status: {
      pending_pickup: "Chờ lấy hàng",
      ready_to_pickup: "Sẵn sàng lấy hàng",
      picked_up: "Đã nhận hàng",
      shipped: "Đã nhận hàng",
      delivering: "Đang giao",
      failed: "Giao thất bại",
      delivered: "Đã giao",
      cancelled: "Đã hủy",
      returned: "Đã trả",
    },
    pending: {
      title: "Đơn chưa giao",
      subtitle: "Có {count} đơn đang chờ bạn xử lý",
      itemsCount: "{count} sản phẩm",
      empty: "Hiện chưa có đơn nào",
      claimed: "Đã nhận {code}",
      claimFailed: "Nhận đơn thất bại: {msg}",
    },
    delivered: {
      title: "Đơn đã giao",
      subtitle: "Đã hoàn thành {count} đơn",
      deliveredAt: "Thời gian giao",
      empty: "Chưa có đơn nào được giao",
      loading: "Đang tải đơn đã giao…",
      last7: "7 ngày qua",
      last30: "30 ngày qua",
      last90: "90 ngày qua",
    },
    earnings: {
      title: "Thu nhập",
      subtitle: "Theo dõi thu nhập từ các đơn đã giao",
      totalLifetime: "Tổng cộng dồn",
      thisMonth: "Tháng này",
      today: "Hôm nay",
      chartTitle: "Biểu đồ thu nhập",
      chartSubtitle: "Tổng phí ship theo thời gian",
    },
    metrics: {
      toPickup: "Cần lấy hàng",
      delivering: "Đang giao",
      delivered: "Đã giao",
      estimateFee: "Phí ship dự kiến",
      totalFee: "Tổng phí ship",
      successRate: "Tỉ lệ thành công",
    },
    actions: {
      call: "Gọi khách",
      claim: "Nhận đơn",
      markDelivered: "Đã giao",
      pickup: "Lấy hàng",
      startDelivery: "Bắt đầu giao",
      viewDetail: "Chi tiết đơn hàng",
    },
    orderDetail: {
      title: "Chi tiết đơn hàng",
      close: "Đóng",
      cancel: "Đóng",
      customer: "Thông tin khách hàng",
      items: "Sản phẩm",
      quantity: "Số lượng: {qty}",
      subtotal: "Tạm tính",
      shippingFee: "Phí ship",
      total: "Tổng cộng",
      paymentMethod: "Phương thức thanh toán",
      paymentMethods: {
        cod: "Thanh toán khi nhận hàng (COD)",
        vnpay: "VNPay",
        momo: "Ví MoMo",
        paypal: "PayPal",
        bank: "Chuyển khoản",
      },
      proofPhoto: "Ảnh xác nhận giao hàng",
      proofHint: "Tối thiểu {min}, tối đa {max} ảnh",
      proofRequired: "Cần ảnh xác nhận trước khi hoàn tất đơn",
      uploadPhoto: "Đẩy ảnh lên",
      replacePhoto: "Đổi ảnh khác",
      removePhoto: "Xoá ảnh",
      openCamera: "Mở camera chụp",
      openCameraDesc: "Chụp ảnh giao hàng tại chỗ",
      retakeCamera: "Chụp lại bằng camera",
      fromGallery: "Chọn từ thư viện",
      fromGalleryDesc: "Lấy ảnh có sẵn trong máy",
      replaceFromGallery: "Đổi ảnh khác từ thư viện",
      recipientName: "Tên người nhận",
      recipientPlaceholder: "VD: Nguyễn Văn A",
      codCollected: "Số tiền COD đã thu",
      codHelp: "Phải khớp đúng {amount} cho đơn COD",
      codDue: "COD phải thu: {amount}",
      codMismatch: "Số tiền COD đã thu chưa khớp tổng đơn.",
      codNotApplicable: "Đơn không phải COD nên không thu thêm tiền.",
      submitting: "Đang gửi…",
      retry: "Thử lại",
      genericError: "Có lỗi xảy ra, vui lòng thử lại.",
      invalidStateTrans: "Trạng thái hiện tại không cho phép thao tác này.",
      invalidStateTransition: "Trạng thái hiện tại không cho phép thao tác này.",
      forbidden: "Bạn không có quyền thao tác đơn này.",
      pickedUpToast: "Đã lấy hàng cho đơn {code}",
      startedToast: "Đã bắt đầu giao đơn {code}",
      failedToast: "Đã đánh dấu thất bại đơn {code}",
      complete: "Hoàn tất đơn hàng",
      tabComplete: "Hoàn tất",
      completedToast: "Đơn hàng {code} đã hoàn tất",
      fail: "Đánh dấu thất bại",
      tabFail: "Thất bại",
      failReason: "Lý do thất bại",
      failReasons: {
        not_home: "Khách không có nhà",
        refused: "Khách từ chối nhận",
        wrong_address: "Sai địa chỉ",
        damaged: "Hàng hư hỏng",
        other: "Lý do khác",
      },
      failNote: "Ghi chú thêm",
      failNotePlaceholder: "Mô tả thêm chi tiết để CSKH theo dõi (không bắt buộc).",
    },
    profile: {
      title: "Hồ sơ",
      subtitle: "Thông tin tài khoản shipper",
      defaultName: "Shipper",
      personalInfo: "Thông tin cá nhân",
      personalInfoSub: "Thông tin tài khoản",
      noPhone: "Chưa cập nhật SĐT",
      role: "Vai trò",
      roleCourier: "Nhân viên giao hàng",
      status: "Trạng thái",
      performance: "Hiệu suất",
      performanceSub: "Theo dõi chỉ số làm việc",
      totalDelivered: "Đơn đã giao",
      successRate: "Tỉ lệ thành công",
      avgRating: "Đánh giá",
      activeDays: "Ngày hoạt động",
    },
  },
};
