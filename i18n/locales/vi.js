export default {
    common: {
        welcome: "Chào mừng",
        login: "Đăng nhập",
        logout: "Đăng xuất",
        register: "Đăng ký",
        signIn: "Đăng nhập",
        or: "Hoặc",
        and: "và",
        email: "Email",
        emailAddress: "Địa chỉ Email",
        password: "Mật khẩu",
        confirmPassword: "Xác nhận mật khẩu",
        createAccount: "Tạo tài khoản",
        alreadyHaveAccount: "Đã có tài khoản?",
        phoneNumber: "Số điện thoại",
        firstName: "Tên",
        lastName: "Họ",
        support: "Hỗ trợ",
        unknown: "Không xác định",
        back: "Quay lại",
    },
    role: {
        default: "Mặc định",
        admin: "Quản trị viên",
        manager: "Quản lý",
        customer: "Khách hàng",
        new_customer: "Khách hàng mới",
        accountant: "Kế toán",
        courier: "Người giao hàng",
    },
    sidebar: {
        brandName: "IrusGear",
        searchPlaceholder: "Tìm kiếm...",
        sections: {
            main: "Chính",
            communication: "Liên lạc",
        },
        menu: {
            home: "Trang chủ",
            orders: "Đơn hàng",
            documentation: "Tài liệu",
            mapOverview: "Bản đồ",
            statistics: "Thống kê",
            inbox: "Hộp thư",
            couriers: "Người giao hàng",
            settings: "Cài đặt",
            help: "Trợ giúp",
            products: "Sản phẩm",
        },
        capacity: {
            title: "Dung lượng sử dụng",
            description: "Bạn đã sử dụng {percent}% dung lượng.",
            upgradeBtn: "Nâng cấp",
        },
    },
    page_titles: {
        home: "Trang chủ",
        login: "Đăng nhập",
        forgotPassword: "Quên mật khẩu",
        register: "Đăng ký",
        dashboard: "Bảng điều khiển",
        verify: "Xác minh tài khoản",
        contact: "Liên hệ",
    },
    dashboard: {
        welcome: "Chào mừng trở lại",
        overview: "Tổng quan",
        stats: {
            totalRevenue: "Tổng doanh thu",
            totalOrders: "Tổng đơn hàng",
            totalCustomers: "Tổng khách hàng",
            totalProducts: "Tổng sản phẩm",
            vsLastMonth: "so với tháng trước",
        },
        recentOrders: {
            title: "Đơn hàng gần đây",
            viewAll: "Xem tất cả",
            orderId: "Mã đơn",
            customer: "Khách hàng",
            date: "Ngày",
            amount: "Số tiền",
            status: "Trạng thái",
            noOrders: "Chưa có đơn hàng nào",
        },
        orderStatus: {
            pending: "Chờ xử lý",
            processing: "Đang xử lý",
            shipped: "Đã giao",
            delivered: "Hoàn thành",
            cancelled: "Đã hủy",
        },
        salesChart: {
            title: "Doanh thu theo tháng",
            revenue: "Doanh thu",
        },
        topProducts: {
            title: "Sản phẩm bán chạy",
            product: "Sản phẩm",
            sold: "Đã bán",
            revenue: "Doanh thu",
        },
    },
    login: {
        newUser: "Người dùng mới?",
        createAccount: "Tạo tài khoản",
        forgotPassword: "Quên mật khẩu của bạn?",
        joinSocial: "Tham gia với tài khoản mạng xã hội yêu thích của bạn",
        terms: "Bằng cách đăng nhập bằng tài khoản, bạn đồng ý với các",
        termsLink: "Điều khoản dịch vụ",
        privacyLink: "Chính sách bảo mật",
        rememberMe: "Ghi nhớ đăng nhập",
        memberTitle: "Trở thành thành viên",
        memberSubtitle: "Để không bỏ lỡ các ưu đãi hấp dẫn",
        benefitDiscount: "Chiết khấu đến 5%",
        benefitDiscountDesc: "khi mua các sản phẩm tại IRUS Gear",
        benefitShipping: "Miễn phí giao hàng",
        benefitShippingDesc: "cho thành viên VIP và đơn hàng từ 500.000đ",
        benefitBirthday: "Tặng voucher sinh nhật",
        benefitBirthdayDesc: "đến 300.000đ cho khách hàng thành viên",
        benefitTradeIn: "Trợ giá thu cũ lên đời",
        benefitTradeInDesc: "đến 500.000đ",
        benefitRankUp: "Thăng hạng nhận voucher",
        benefitRankUpDesc: "đến 200.000đ",
        benefitBusiness: "Đặc quyền doanh nghiệp:",
        benefitBusinessDesc: "Chiết khấu đến 10% dành riêng cho khách hàng B2B",
        viewMemberPolicy: "Xem chi tiết chính sách ưu đãi thành viên",
        loginSuccess: "Đăng nhập thành công!",
    },
    register: {
        title: "Đăng ký",
        subtitle: "Trở thành thành viên ngay hôm nay",
        quickRegister: "Đăng ký nhanh bằng",
        orInfo: "Hoặc điền thông tin",
        personalInfo: "Thông tin cá nhân",
        fullName: "Họ và tên",
        enterFullName: "Nhập họ và tên",
        dob: "Ngày sinh",
        phone: "Số điện thoại",
        enterPhone: "Nhập số điện thoại",
        email: "Email",
        enterEmail: "Nhập email",
        security: "Bảo mật",
        password: "Mật khẩu",
        confirmPassword: "Xác nhận mật khẩu",
        enterPassword: "Mật khẩu",
        reEnterPassword: "Nhập lại mật khẩu",
        promo: "Nhận tin khuyến mãi từ IrusGear",
        
        roleSwitch: "Học sinh - Sinh viên / Giáo viên",
        youAre: "Bạn là:",
        student: "Học sinh - SV",
        teacher: "Giáo viên",
        schoolLevel: "Cấp học",
        chooseLevel: "Chọn cấp học",
        university: "Đại học / Cao đẳng",
        highschool: "Trung học phổ thông",
        secondary: "Trung học cơ sở",
        schoolName: "Tên trường",
        enterSchoolName: "Nhập tên trường",
        uploadID: "Tải ảnh thẻ (Mặt trước & sau)",
        frontSide: "Mặt trước",
        backSide: "Mặt sau",
        uploadDragDrop: "Kéo thả hoặc",
        uploadSelect: "Chọn file",
        uploadFormat: "Định dạng: .JPG, .PNG, .PDF - Tối đa 5 MB",

        businessSwitch: "Tôi là Khách hàng Doanh nghiệp",
        taxCode: "Mã số thuế",
        enterTaxCode: "Nhập mã số thuế",
        companyName: "Tên công ty",
        enterCompanyName: "Nhập tên công ty",
        companyAddress: "Địa chỉ công ty",
        enterCompanyAddress: "Nhập địa chỉ công ty",
        companyEmail: "Địa chỉ email công ty",
        enterCompanyEmail: "Nhập địa chỉ email công ty",
        authorizedCCCD: "CCCD người được uỷ quyền",
        enterAuthorizedCCCD: "Nhập CCCD người được uỷ quyền",
        note: "Ghi chú",
        optional: "(Không bắt buộc)",
        enterNote: "Nhập ghi chú",
        businessLicense: "Giấy phép kinh doanh",
        authorizationLetter: "Giấy uỷ quyền",
        viewSample: "Xem file mẫu",
        
        backBtn: "Quay lại",
        registerBtn: "Đăng ký",
        agreePolicy: "Bằng việc đăng ký, bạn đồng ý với",
        terms: "Điều khoản",
        and: "&",
        policy: "Chính sách",
        backToLogin: "Quay lại đăng nhập",
        goBack: "Quay lại",
        rememberPassword: "Bạn nhớ mật khẩu của mình?",
        businessCustomer: "Khách hàng doanh nghiệp",
        register: "Đăng ký",
        agreeWith: "Tôi đồng ý với",
        uploadAction: "Tải file lên",
    },
    footer: {
        slogan: {
            line1: "Gear tốt nhất",
            line2: "cho phong cách",
            line3: "của bạn"
        },
        locations: {
            hcm: {
                name: "Hồ Chí Minh",
                address1: "123 Nguyễn Huệ,",
                address2: "Quận 1, TP.HCM"
            },
            hanoi: {
                name: "Hà Nội",
                address1: "456 Phố Huế,",
                address2: "Hai Bà Trưng, Hà Nội"
            }
        },
        seeOnMap: "XEM BẢN ĐỒ",
        newsletter: {
            question: "Muốn nhận tin mới nhất?",
            signup: "ĐĂNG KÝ NHẬN TIN"
        },
        followUs: "Theo dõi chúng tôi",
        copyright: "© 2026 IrusGear. Bảo lưu mọi quyền.",
        description: "Nền tảng mua sắm công nghệ cao cấp. Sản phẩm chính hãng, bảo hành minh bạch, trải nghiệm mượt mà từ UI đến dịch vụ."
    },
    validate: {
        passwordsNotMatch: "Mật khẩu không khớp",
        required: "Vui lòng nhập thông tin này",
        invalid_date: "Ngày không hợp lệ",
        numeric_only: "Vui lòng chỉ nhập số",
        phone_min: "Số điện thoại phải có ít nhất 10 số",
        phone_max: "Số điện thoại không được quá 12 số",
        email_invalid: "Email không đúng định dạng",
        password_min: "Mật khẩu phải có ít nhất 6 ký tự",
        password_not_match: "Mật khẩu xác nhận không khớp",
        max_length: "Nội dung không được vượt quá {max} ký tự",
        xss_invalid: "Vui lòng không nhập các ký tự đặc biệt (&lt;, &gt;, &#123;, &#125;, ;)",
    },
    error: {
        oops: "Oops",
        pageNotFound: "Trang bạn tìm kiếm không tồn tại",
        cantFind: "chúng tôi không tìm thấy trang đó...",
        dragMe: "kéo tôi",
        backHome: "Về trang chủ",
    },
    verify: {
        title: "Xác minh tài khoản",
        subtitle: "Vui lòng nhập mã xác minh đã được gửi đến email của bạn",
        description: "Mã xác thực đã được gửi đến email",
        otpSubtitle: "Nhập mã xác thực",
        verifyBtn: "Xác minh",
        resendCode: "Bạn chưa nhận được mã? ",
        resendBtn: "Gửi lại",
    },
    restorePassword: {
        recoverTitle: "Khôi phục mật khẩu",
        recoverSubtitle: "Nhập email đã đăng ký để nhận mã xác minh",
        checkSpam: "Kiểm tra hộp thư Spam",
        checkSpamDesc: "Nếu không thấy mã trong Hộp thư đến, hãy kiểm tra mục Spam hoặc Quảng cáo.",
        secureTip: "Bảo mật thông tin",
        secureTipDesc: "Tuyệt đối không chia sẻ mã xác thực (OTP) cho bất kỳ ai, kể cả nhân viên hỗ trợ.",
        needHelp: "Cần hỗ trợ thêm?",
        needHelpDesc: "Liên hệ cho chúng tôi nếu bạn không thể truy cập email này.",
        recover: "Khôi phục",
        forgotPassword: "@:page_titles.forgotPassword",
        verification: "Xác minh",
        resetPassword: "Đặt lại mật khẩu",
        forgotPasswordSubtitle: "Đừng lo lắng, chúng tôi ở đây để hỗ trợ bạn.",
        otpSubtitle: "Vui lòng nhập mã xác minh đã được gửi đến email của bạn",
        newPasswordSubtitle: "Vui lòng nhập mật khẩu mới cho tài khoản của bạn",
        forgotPasswordNote: "Chúng tôi sẽ gửi mã xác nhận đến email của bạn.",
        sendCode: "Gửi mã xác nhận",
        newPassword: "Mật khẩu mới",
        confirmNewPassword: "Xác nhận mật khẩu mới",
        resetPasswordBtn: "Đổi mật khẩu",
    },
    mobile: {
        header: {
            marquee: "🎉 Giảm đến 50% toàn bộ sản phẩm • 📱 Trả góp 0% lãi suất • 🚚 Giao hàng miễn phí • 🔥 Flash Sale mỗi ngày • 🎉 Giảm đến 50% toàn bộ sản phẩm • 📱 Trả góp 0% lãi suất • 🚚 Giao hàng miễn phí • 🔥 Flash Sale mỗi ngày •",
        },
        bottomNav: {
            home: "Trang chủ",
            category: "Danh mục",
            promo: "Khuyến mãi",
            search: "Tìm kiếm",
            account: "Tài khoản",
        },
        categories: {
            phone: "Điện thoại",
            tablet: "Tablet",
            laptop: "Laptop",
            watch: "Đồng hồ",
            audio: "Âm thanh",
            accessories: "Phụ kiện",
            pc: "PC",
            tv: "Tivi",
            tradeIn: "Thu cũ",
            used: "Hàng cũ",
            deals: "Khuyến mãi",
            homeAppliance: "Gia dụng",
        },
        flashSale: {
            title: "FLASH SALE",
            updating: "Đang cập nhật...",
        },
        sections: {
            featuredPhones: "Điện thoại nổi bật",
            laptop: "Laptop",
            watch: "Đồng hồ",
            tv: "Tivi",
            homeAppliance: "Đồ gia dụng",
            viewAll: "Xem tất cả",
        },
        usedGoods: {
            title: "HÀNG CŨ",
            viewAll: "Xem tất cả",
            phoneUsed: "Điện thoại cũ",
            tabletUsed: "Máy tính bảng cũ",
            macbookUsed: "MacBook cũ",
            laptopUsed: "Laptop cũ",
            headphoneUsed: "Tai nghe cũ",
            speakerUsed: "Loa cũ",
            watchUsed: "Đồng hồ cũ",
            applianceUsed: "Đồ gia dụng cũ",
            accessoryUsed: "Phụ kiện cũ",
            monitorUsed: "Màn hình cũ",
            tvUsed: "Tivi cũ",
            cableUsed: "Cáp sạc cũ",
        },
        footer: {
            companyDesc: "Nền tảng mua sắm công nghệ cao cấp. Sản phẩm chính hãng, bảo hành minh bạch, trải nghiệm mượt mà.",
            address: "123 Nguyễn Văn Linh, Quận 7, TP.HCM",
            phone: "1800 1234 (Miễn phí)",
            copyright: "© 2026 IrusGear. Bảo lưu mọi quyền.",
            terms: "Điều khoản",
            privacy: "Bảo mật",
            cookies: "Cookies",
        },
    },
    contact: {
        pageTitle: "Liên hệ",
        heroTitle: "Liên hệ với chúng tôi",
        heroSubtitle: "Chúng tôi rất mong được nghe từ bạn. Dù bạn có câu hỏi về sản phẩm, giá cả hay bất cứ điều gì — đội ngũ của chúng tôi luôn sẵn sàng hỗ trợ.",
        form: {
            title: "Gửi tin nhắn cho chúng tôi",
            name: "Họ và tên",
            namePlaceholder: "Nhập họ và tên của bạn",
            email: "Địa chỉ Email",
            emailPlaceholder: "Nhập email của bạn",
            phone: "Số điện thoại",
            phonePlaceholder: "Nhập số điện thoại",
            subject: "Chủ đề",
            subjectPlaceholder: "Bạn cần hỗ trợ về vấn đề gì?",
            message: "Tin nhắn",
            messagePlaceholder: "Viết tin nhắn của bạn tại đây...",
            submit: "Gửi tin nhắn",
            sending: "Đang gửi...",
            successTitle: "Đã gửi thành công!",
            successMsg: "Cảm ơn bạn đã liên hệ. Chúng tôi sẽ phản hồi trong vòng 24 giờ.",
            sendAnother: "Gửi tin nhắn khác",
        },
        info: {
            title: "Thông tin liên hệ",
            subtitle: "Liên hệ với chúng tôi qua các kênh sau",
            address: "Địa chỉ",
            addressValue: "123 Nguyễn Văn Linh, Quận 7, TP. Hồ Chí Minh",
            phone: "Điện thoại",
            phoneValue: "1800 1234 (Miễn phí)",
            phoneValue2: "+84 28 1234 5678",
            email: "Email",
            emailValue: "support{'@'}irusgear.com",
            emailValue2: "sales{'@'}irusgear.com",
            hours: "Giờ làm việc",
            hoursValue: "Thứ 2 - Thứ 6: 8:00 - 21:00",
            hoursValue2: "Thứ 7 - Chủ nhật: 9:00 - 18:00",
        },
        map: {
            title: "Tìm chúng tôi trên bản đồ",
        },
        faq: {
            title: "Câu hỏi thường gặp",
            q1: "Các hình thức giao hàng là gì?",
            a1: "Chúng tôi miễn phí giao hàng cho đơn hàng trên 500.000đ. Giao hàng tiêu chuẩn từ 2-5 ngày làm việc, và giao hàng nhanh trong ngày hôm sau.",
            q2: "Chính sách đổi trả như thế nào?",
            a2: "Chúng tôi chấp nhận đổi trả trong vòng 30 ngày kể từ ngày mua. Sản phẩm phải còn nguyên trạng và bao bì. Hoàn tiền được xử lý trong 5-7 ngày làm việc.",
            q3: "Sản phẩm có được bảo hành không?",
            a3: "Có, tất cả sản phẩm đều có bảo hành chính hãng. Gói bảo hành mở rộng cũng có sẵn để mua thêm.",
            q4: "Làm sao để theo dõi đơn hàng?",
            a4: "Sau khi đơn hàng được giao, bạn sẽ nhận được mã theo dõi qua email. Bạn cũng có thể theo dõi đơn hàng trong mục 'Đơn mua' của tài khoản.",
        },
    },
}
